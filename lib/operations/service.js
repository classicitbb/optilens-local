...
async function insertSupplierRecord({ pool, mailboxMessageId, eventId, attachmentId, rowNumber, rule, item, sourceResult, mapping, state, errorDetail = null }) {
  ...
  const result = await pool.request()
    ...
    .query(`
      IF EXISTS (SELECT 1 FROM ops.SupplierRecords WHERE event_id = @event_id AND attachment_id = @attachment_id AND row_number = @row_number)
      BEGIN
        ...
        SELECT record_id FROM ops.SupplierRecords WHERE event_id = @event_id AND attachment_id = @attachment_id AND row_number = @row_number;
      END
      ELSE
      BEGIN
...
async function refreshLiveOrderStatuses(records) {
  ...
  const orderIds = [...new Set(rows.map((row) => Number(row.internal_order_id)).filter((id) => Number.isSafeInteger(id) && id > 0))];
  ...
  for (let offset = 0; offset < orderIds.length; offset += 500) {
    ...
    `);
    for (const row of result.recordset) current.set(Number(row.internal_order_id), row);
  }
...
async function autoArchiveAppliedSupplierMessage({ mailboxMessageId, actorUserId }) {
  ...
  const result = await pool.request()
    ...
    .query(`
      ...
      WHERE mailbox_message_id = @mailbox_message_id;
    `);
  ...
  const records = await pool.request()
    ...
    .query(`
      SELECT COUNT(*) AS actionable_count,
             SUM(CASE WHEN parse_state = N'APPLIED' THEN 1 ELSE 0 END) AS applied_count
      FROM ops.SupplierRecords WHERE mailbox_message_id = @mailbox_message_id;
    `);
  ...
  const records = await pool.request()
    ...
    .query(`
      SELECT TOP (@limit)
        ...
        r.created_at, a.status AS action_status
      FROM ops.SupplierRecords r
      INNER JOIN ops.MailboxMessages m ON m.mailbox_message_id = r.mailbox_message_id AND m.archive_state = N'IN_INBOX'
      LEFT JOIN ops.Actions a ON a.action_id = r.action_id
      WHERE (@supplier_code IS NULL OR r.supplier_code = @supplier_code)
        AND (@mailbox_message_id IS NULL OR r.mailbox_message_id = @mailbox_message_id)
      ORDER BY r.created_at DESC, r.row_number;
    `);
...
async function listStatusMappings() {
  ...
  `);
  const source = await getSourcePool();
  const statuses = await source.request().query(`
  ...
  `);
  return { mappings: mappings.recordset, statusItems: statuses.recordset };
}
