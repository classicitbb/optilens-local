  ...
  const pool = await getSourcePool();
    ...
    for (const row of aliases) {
      ...
      records.push({
        ...
        mfType,
        category: mfType,
        suppliers: [...priceRow.suppliers].sort((left, right) => left.localeCompare(right)),
    ...
    const matchedPricingKeys = new Set(records.map((row) => pricingKey(row.materialDescription, row.mfType, row.styleDescription)));
...
module.exports = { syncRxCatalog, syncRxCoatings, canonicalMaterial, CATALOG_FILE, ADDONS_FILE, COATINGS_FILE, METADATA_FILE };
