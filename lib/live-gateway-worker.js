...
const AGENT_VERSION = '2026-07-11.1';
...
class LiveGatewayWorker {
  ...
  start(creds) {
    ...
    this.creds = { baseUrl: creds.baseUrl || '', apiKey: creds.apiKey, innovaApi: creds.innovaApi || null };
  ...
  capabilities() {
    return OPERATIONS.filter((operation) => operation !== 'innovations.customer_rx_order_status' || !!this.creds?.innovaApi?.bearerToken);
  }
  ...
  async processRequest(request) {
    ...
    try {
      const data = await Promise.race([
        dispatch(request, { innovaApi: this.creds?.innovaApi || null }),
