...
async function runOnce(creds, { max = 3 } = {}) {
  ...
  for (let i = 0; i < max; i += 1) {
    ...
    try {
      ...
      const filename = released.released[0];
      // Dropping the file is not the same as Innova accepting it: the watcher
      // renames anything it cannot parse to <name>.bad. Wait for that verdict
      // rather than reporting a copy as a delivered order.
      const outcome = await stockGenerator.checkReleaseOutcome(filename);
      const ok = outcome !== 'rejected';
      await complete(creds, {
        id: sub.id, ok, transport: 'file_drop', filename, attempts: sub.attempts,
        ...(ok ? {} : { error: `Innova rejected ${filename} (renamed .bad in the Incoming folder).` }),
        receipt: { outcome },
      });
      syncLog.write('stock_submission.finished', { id: sub.id, transport: 'file_drop', ok, outcome, file: filename });
      processed.push({ id: sub.id, ok, outcome, transport: 'file_drop', filename });
    } catch (err) {
