...
function buildSoldItemsBatch() {
  return `
    ...
    -- Grouped by SKU *and* CategoryType, not SKU alone. The same SKU legitimately
    -- appears in different roles with different economics: 0034804914 sells as Edging
    -- (CategoryType 7) at cost 6 / price 10 and as a Lens (CategoryType 1) at cost 30.6
    -- / price 70. Collapsing them takes the highest cost against the lowest price and
    -- invents a violation that does not exist.
    SELECT
    ...
    GROUP BY LTRIM(RTRIM(d.SKU)), d.CategoryType;
    ...
    CREATE CLUSTERED INDEX IX_sold_items_key ON #sold_items(itemKey, categoryType);
...
/**
 * The item master (vw.StockItems, LensItem, MiscItems) is not mirrored, so on the mirror
 * profile this would fail with a raw "Invalid object name". Say what is actually wrong.
 */
async function assertItemMasterAvailable(pool) {
  const probe = await pool.request().query(`
    SELECT
      (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'LensItem') AS lensItem,
      (SELECT COUNT(*) FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_NAME = 'StockItems') AS stockItemsView;
  `);
  const row = probe.recordset[0] || {};
  if (!Number(row.lensItem) || !Number(row.stockItemsView)) {
    const profile = await getActiveSourceProfile();
    throw Object.assign(
      new Error(
        `The item cost register needs the item master, which the "${profile}" source profile does not ` +
        "carry — the Innovations mirror holds no LensItem or StockItems data. Switch the source " +
        "profile to live to use this report."
      ),
      { statusCode: 503 }
    );
  }
...
async function queryDataQuality() {
  ...
  await assertItemMasterAvailable(pool);
