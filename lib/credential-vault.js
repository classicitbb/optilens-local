...
function normalizeVaultData(data) {
  ...
  // A vault is operator-owned after setup. In particular, an empty category or
  // a missing former template entry is intentional and must never be recreated
  // merely because the vault is read by another service or after a lock cycle.
  return { data, changed: false };
}
...
/**
 * The RX and stock-order generators share Innovations' watched incoming
 * folder.  Keep the operator override in the vault, while retaining the
 * checked-in RX configuration as a safe fallback for existing installations.
 */
function innovationsIncomingFolderFromVault(entryFinder = findVaultEntry) {
  const entry = entryFinder("Other", ["innovations incoming folder", "innovations file drop"]);
  if (!entry) return null;
  const fields = fieldMap(entry);
  const folder = String(fields.incomingfolder || fields.incomingpath || fields.folder || "").trim();
  return folder || null;
}
...
function mailboxFromVault(matchers = ["classic visions orders", "classic visions", "orders mailbox"]) {
  ...
  return {
    ...
    smtpHost: String(fields.smtphost || fields.smtpserver || "").trim(),
    smtpPort: Number(fields.smtpport || 0),
    smtpSecure: String(fields.smtpsecure || "").trim().toLowerCase() === "true"
  };
}
...
module.exports = {
  ...
  findCvApiVaultEntry,
  findVaultEntry,
  innovationsIncomingFolderFromVault,
  mailboxFromVault,
