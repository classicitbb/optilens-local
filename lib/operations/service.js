...
async function createFixtureWorkflow({ filename, contentType, buffer, actorUserId, storagePath }) {
  ...
  for (const item of parsed.items) {
    const match = developmentMatch(item);
    if (match) {
      actions.push(await createProposedAction({ eventId: event.event_id, item, match, actorUserId }));
      continue;
    }
    exceptions += 1;
    const outcome = item.warning ? "UNKNOWN_SUPPLIER_STATUS" : "UNMATCHED";
    await pool.request()
      ...
      .input("detail", safeJson({ item, match: { matched: false, outcome, targetReference: item.supplierReference } }))
