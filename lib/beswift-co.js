...
const { resolveDeliveryTerms, getBeswiftOptionIndexHints } = require("./standards-catalog");
...
async function getCommercialInvoicePreview(sessionId) {
  ...
  const items = composed.items.map((item, index) => {
    ...
    const defaultSpecification = commercialDescription;
    return {
  ...
  const declarationOverride = text(headerOverrides.declaration);
  const contactName = [h.FirstName, h.Surname].map(text).filter(Boolean).join(" ");
  const importerAddr = [h.Line1, h.Line2, h.Locality, h.PostCode].map(text).filter(Boolean).join(", ");
  ...
  const grossWeightKg = grossWeightLbsOverride
    ? lbsToKg(numberFromText(grossWeightLbsOverride, 0))
    : round(Number(packaging.actualGrossKg || 0), 3);
  ...
  const customerOrderNoDefault = [...composed.orderRefs].join(", ");
  ...
  return {
    ...
    packaging: {
      ...
      grossWeightLbs: grossWeightLbsOverride,
