...
async function composeShipmentItems(lines, warnings) {
  ...
  const entries = enriched.map((entry) => {
    ...
    const keyParts = [entry.displayName, entry.unitPrice.toFixed(2), entry.hsCode, entry.countryOfOrigin, refLabel];
...
async function buildCommercialInvoiceLines(session, lines, warnings) {
  ...
  for (const entry of adjusted) {
    const keyParts = [entry.displayName, entry.unitPrice.toFixed(2), entry.hsCode, entry.countryOfOrigin];
