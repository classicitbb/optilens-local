...
async function runOnce() {
  ...
  try {
    ...
    await fetch(`${root}/qbo/commands/result`, { method: 'POST', headers, body: JSON.stringify({ id: command.id, command: command.command, status: 'completed', result_sanitized: result }) });
    return { claimed: true, command: command.command, ok: true };
  } catch (error) {
    await fetch(`${root}/qbo/commands/result`, { method: 'POST', headers, body: JSON.stringify({ id: command.id, command: command.command, status: 'error' }) });
