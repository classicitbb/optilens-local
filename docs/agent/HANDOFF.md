...
- Status: In progress — RX Capture Milestones 1 and 2 deployed; authenticated acceptance verification pending
- Last synchronized: 2026-09-22

## Objective and current state

Business Metrics now has a dedicated **OpenAI API — GPT-5.6 Luna** Chat API preset. It selects the existing OpenAI-compatible endpoint and the exact `gpt-5.6-luna` model ID; the generic OpenAI choice is retained for custom model IDs. Source syntax, the focused assistant suite (5/5), and `npm run check` pass. The saved application setting was intentionally left at its prior `gpt-4o-mini` value because the required read-only model-list preflight failed from Node with `UNABLE_TO_VERIFY_LEAF_SIGNATURE`. Repair the approved workstation/Node CA trust chain, then verify that the account exposes Luna through `GET /models` before saving the new preset. Do not disable TLS verification or send a billable completion merely to test the migration.
...
RX Capture review refinements are complete locally but not deployed: the recent-orders listener no longer receives `Array.map` metadata as an event handler; capture now requires a source-validated ERP customer selected before the image; patient names are normalized to `LAST, FIRST`; the review form uses one flat OD/OS prescription table with PD and optical-center-height values; frame fields follow patient fields; and production configuration is gated on an explicit review save. The server persists the chosen customer and review confirmation through additive migration `045-rx-capture-review-workflow.sql`, forces the remote operator to the authenticated user, and provides a guarded catalogue suggestion (photochromic without a colour defaults to gray; clear defaults to SR-coated) that still requires confirmation of the exact alias. Affected files are `database/045-rx-capture-review-workflow.sql`, `lib/migrations.js`, `lib/rx-capture/{alias-resolver,normalized-order,routes,service}.js`, `server.js`, `public/rx-capture.{html,js}`, `public/styles/pages/rx-capture.css`, and `test/rx-capture.test.js`. `node --test test/rx-capture.test.js` passed 16/16, `node --check server.js` passed, `git diff --check` passed, and `npm test` exited 0 in the local runner. Blocker: production deployment and an authenticated external-browser pass have not been authorized for this revision. Next action: after explicit production-deployment approval, run the guarded host update so migration 045 applies, then use external Edge or Chrome to direct-type a customer search and patient name, save a review, and confirm recent orders, the compact table, configuration gate, and alias suggestion without creating a real image capture.
