...
async function getBootstrapState() {
  ...
  const result = await pool.request().query(`
    ...
    BEGIN
      SELECT
        CAST(1 AS bit) AS needs_migration,
        CAST(1 AS bit) AS needs_bootstrap,
        CAST(0 AS int) AS credential_count;
    END
    ELSE
    BEGIN
      SELECT
        CAST(0 AS bit) AS needs_migration,
        CASE WHEN COUNT(*) = 0 THEN CAST(1 AS bit) ELSE CAST(0 AS bit) END AS needs_bootstrap,
  ...
  return {
    needsMigration: Boolean(row.needs_migration),
    needsBootstrap: Boolean(row.needs_bootstrap),
...
async function bootstrapAdmin(payload) {
  ...
  if (state.needsMigration) {
    const error = new Error("Authentication tables are not installed. Run /api/admin/migrate before creating the first admin.");
...
async function authenticateUser(usernameValue, password) {
  ...
  const state = await getBootstrapState();
  if (state.needsMigration) {
    const error = new Error("Authentication tables are not installed. Run /api/admin/migrate, then sign in again.");
    error.statusCode = 503;
    throw error;
  }
