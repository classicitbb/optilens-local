...
const { FRAME_DEFAULTS } = require("../../public/rx-capture-validation");

const FRAME_MODEL_PLACEHOLDER = "FRAME TO FOLLOW";
...
function normalizeResolution(value) {
  ...
  return {
    ...
    frameMode,
    frameMounting: text(source.frameMounting || "2", "Frame mounting", 20, true),
...
// The distance PDs are what is sent; a binocular PD only stands in, split
// evenly, for an eye whose distance PD is blank.
function monocularPd(pd) {
  const binocular = optionalDecimal(pd.binocular, 1, 100, "PD");
  const half = binocular === null ? null : binocular / 2;
  return {
    od: optionalDecimal(pd.od, 0.1, 100, "OD PD") ?? half,
    os: optionalDecimal(pd.os, 0.1, 100, "OS PD") ?? half
  };
}
...
function buildFrame(order, resolution) {
  ...
  // Edged jobs need a box size; blank measurements fall back to the defaults.
  const measure = (key, label) => (edged
    ? oneDecimal(decimal(blank(frame[key]) ? FRAME_DEFAULTS[key] : frame[key], label, 0.1, 100))
    : optionalOneDecimal(optionalDecimal(frame[key], 0.1, 100, label)));
  return {
    source: "NO TRACE - UNCUT",
    status: edged ? "ENCLOSED" : "UNCUT",
    tracing: "NO TRACE",
    // An undetected model stays null on the draft; the lab reads it from the frame.
    model: text(frame.model || (edged ? FRAME_MODEL_PLACEHOLDER : "UNCUT"), "Frame model", 80, true),
    color: text(frame.color || "1", "Frame color", 40, true),
    a: measure("a", "Frame A"),
    b: measure("b", "Frame B"),
    dbl: measure("dbl", "Frame DBL"),
    radAngle: "45.0",
...
module.exports = { FRAME_MODEL_PLACEHOLDER, buildRxCaptureOrder, normalizeResolution };
