...
async function buildPayloadFromShipment(session, draftInput = {}) {
  ...
  const warnings = [];
  const composed = await composeShipmentItems(linesResult.recordset, warnings);
...
async function composeShipmentItems(lines, warnings) {
  ...
  for (const [, orderLines] of byOrder) {
    ...
    for (const row of orderLines) {
      ...
      if (category === LENS_BASE_CATEGORY || LENS_ADDON_CATEGORIES.has(category)) {
    ...
    if (lensParts.length) {
      ...
      composed.push({ name: nameParts.join(" + "), unitPrice: round(total, 2), qty: 1, category: "lens" });
    }

    for (const frame of frameItems) {
      composed.push({ name: frame.name, unitPrice: round(frame.unitPrice, 2), qty: frame.qty, category: "frame" });
    }
    for (const other of otherItems) {
      composed.push({ name: other.name, unitPrice: round(other.unitPrice, 2), qty: other.qty, category: "other" });
    }
  }

  const grouped = new Map();
  for (const entry of composed) {
    const key = `${entry.name}\u241F${entry.unitPrice.toFixed(2)}`;
    if (!grouped.has(key)) {
      grouped.set(key, { name: entry.name, unitPrice: entry.unitPrice, qty: 0, category: entry.category });
    }
    grouped.get(key).qty += entry.qty;
  }

  const groupedItems = [...grouped.values()];

  if (!groupedItems.length) {
    warnings.push("No billable invoice lines were found to build BeSwift items from.");
  }

  await touchCatalogEntries(groupedItems.map((item) => ({ name: item.name, category: item.category })));
  const catalog = await getCatalogEntries(groupedItems.map((item) => item.name));
  ...
  const items = groupedItems.map((item) => {
    ...
    const defaultUom = item.category === "frame" ? "Pieces" : item.category === "lens" ? "Pairs" : "Pieces";
    const defaultWeight = item.category === "lens" ? CONFIG.weights.lensPairUncut : 0;
    const isGlassLens = item.category === "lens" && /\bglass\b/i.test(item.name);
    const defaultHsCode = item.category === "lens"
      ? (isGlassLens ? CONFIG.hsCodes.lensGlass.code : CONFIG.hsCodes.lensOther.code)
      : "";
    const defaultHsDescription = item.category === "lens"
      ? (isGlassLens ? CONFIG.hsCodes.lensGlass.description : CONFIG.hsCodes.lensOther.description)
      : "";

    return {
      name: item.name,
      hsCode: entry?.hsCode || defaultHsCode,
      hsDescription: entry?.hsDescription || defaultHsDescription,
      commercialDescription: entry?.commercialDescription || item.name,
      quantity: item.qty,
      uom: entry?.unitOfMeasure || defaultUom,
      countryOfOrigin: entry?.countryOfOrigin || CONFIG.defaults.countryOfOrigin,
      unitCost: item.unitPrice,
      value: round(item.unitPrice * item.qty, 2),
      weightKg: entry?.weightKg != null ? Number(entry.weightKg) : defaultWeight
    };
  });
