...
const { getDataQuality } = require("./data-quality");
const { getInventorySection, getInventoryTrendsSection } = require("./inventory");
...
const SECTIONS = {
  ...
  // Aggregates a year of invoice lines and resolves every sold SKU against the item
  // master, which lands around 16s. Given its own budget so it is not killed by the
  // shared 20s ceiling on a slow day.
  "data-quality": { needsSource: true, needsApp: false, timeoutMs: 40000 },
  ...
  // Phase 2 analytics, loaded separately by the same tab. The add-power channel
  // join (RxArchive -> LensItem on the six-part key) costs ~8s, so it must not
  // sit in front of the headline numbers, which are ready in ~3s.
  "inventory-trends": { needsSource: false, needsApp: false, timeoutMs: 45000 }
};
...
async function getDetailSection(section) {
  ...
  try {
    const data = await withTimeout(
      HANDLERS[section]({ sourceOnline, appOnline }),
      spec.timeoutMs || SECTION_TIMEOUT_MS,
      `${section} query`
    );
...
const HANDLERS = {
  ...
  async "data-quality"({ sourceOnline }) {
    if (!sourceOnline) return { dataQuality: null };
    const register = await getDataQuality();
    if (!register.online) {
      throw Object.assign(new Error(register.error), { statusCode: 503 });
    }
    return { dataQuality: register };
  },
  ...
  async "inventory-trends"() {
    return { inventoryTrends: await getInventoryTrendsSection() };
  }
};
