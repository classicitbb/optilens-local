...
async function updateAutomationJobStatus(jobId, body = {}) {
  ...
  // "submitted" is terminal too: the beta extension can now carry a run past
  // the review pause and file the certificate with the certifying authority,
  // which is the end of the automation's job — payment is a separate module.
  const completed = ["filled_review", "submitted", "error", "cancelled"].includes(status);
  const pool = await getAppPool();
  const result = await pool.request()
    ...
    await pool.request()
      ...
      .query(`
        ...
        SET app_status = @app_status,
            filled_at = CASE WHEN @app_status IN (N'filled_review', N'submitted') THEN SYSUTCDATETIME() ELSE filled_at END,
            updated_at = SYSUTCDATETIME()
        WHERE co_application_id = @co_application_id
      `);
  }
...
function normalizeJobStatus(value) {
  ...
  return ["claimed", "logging_in", "filling", "paused", "filled_review", "submitted", "error", "cancelled"].includes(status) ? status : "filling";
}
