...
const { archiveSupplierMessage, confirmStatusMapping, createFixtureWorkflow, createSupplierRule, decideAction, deleteSupplierRule, getActionDetail, getExceptionDetail, getMailboxConfiguration, listActions, listExceptions, listMailboxReconciliation, listMailboxWorkspace, listOperations, listStatusMappings, listSupplierRules, resolveException, syncMailbox, updateMailboxConfiguration, updateSupplierRule } = require("./service");
...
async function handleOperationsRoute({ req, res, url, handleApi, readJsonBody, requirePermission }) {
  ...
  const actionDetail = url.pathname.match(/^\/api\/operations\/actions\/([^/]+)$/);
  if (actionDetail && req.method === "GET") {
    await handleApi(res, async () => { await requirePermission(req, "automation.read"); return getActionDetail(actionDetail[1]); });
    return true;
  }
  ...
  const exceptionDetail = url.pathname.match(/^\/api\/operations\/exceptions\/([^/]+)$/);
  if (exceptionDetail && req.method === "GET") {
    await handleApi(res, async () => { await requirePermission(req, "automation.read"); return getExceptionDetail(exceptionDetail[1]); });
    return true;
  }
