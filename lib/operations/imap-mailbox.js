...
const { DIGEST_HEADER } = require("./exception-digest");
...
async function readMailbox({ config, credential, rules, onMessage = async () => {} }) {
  ...
  try {
    ...
    for (const uid of uids) {
      ...
      if (String(parsed.headers?.get("x-optilens-notification") || "").trim().toLowerCase() === DIGEST_HEADER) {
        results.push({ uid, ignored: "system-notification" });
        continue;
      }
