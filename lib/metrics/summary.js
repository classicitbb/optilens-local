...
async function queryOverview(periodKey) {
  ...
  const result = await request.query(`
    ...
    -- 8: stock-lot costing coverage (why stock turn is withheld)
    SELECT COUNT(*) AS lots,
           SUM(CASE WHEN OnHand > 0 THEN 1 ELSE 0 END) AS lotsWithStock,
           SUM(CASE WHEN OnHandCalcTime IS NOT NULL THEN 1 ELSE 0 END) AS lotsCosted
...
function buildDataQuality({ stock, roll }) {
  ...
  const costed = num(stock.lotsCosted);

  if (lots > 0 && costed < lots) {
    out.push({
      id: "stock-uncosted",
      text: `Stock turn withheld — ${fmtInt(costed)} of ${fmtInt(lots)} active stock lots have a calculated on-hand value.`
    });
  }
