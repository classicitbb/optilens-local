...
function status() {
  ...
  return {
    ...
    innovaapi: i ? { baseUrl: i.baseUrl || '', bearerTokenMasked: i.tokenLast4 ? '••••' + i.tokenLast4 : '', updatedAt: i.updatedAt || null } : null,
  };
}
...
function saveInnovaApi(token, { baseUrl, bearerToken }) {
  const key = keyForToken(token);
  if (!key) throw new Error('Locked — unlock first.');
  const c = load();
  const api = c.innovaapi || {};
  if (baseUrl != null) api.baseUrl = String(baseUrl).trim().replace(/\/$/, '');
  if (bearerToken) { api.bearerToken = enc(bearerToken, key); api.tokenLast4 = bearerToken.slice(-4); }
...
function getInnovaApi(token) {
  const key = keyForToken(token);
  if (!key) throw new Error('Locked — unlock first.');
  const api = load()?.innovaapi;
  if (!api?.bearerToken) throw new Error('InnovaAPI bearer token not configured.');
  return { baseUrl: api.baseUrl || 'https://localhost/api/v2', bearerToken: dec(api.bearerToken, key) };
}

function revealInnovaApi(token) {
  const key = keyForToken(token);
  if (!key) throw new Error('Locked — unlock first.');
  const api = load()?.innovaapi;
  return { baseUrl: api?.baseUrl || 'https://localhost/api/v2', bearerToken: api?.bearerToken ? dec(api.bearerToken, key) : '' };
}

module.exports = { isInitialised, setPassphrase, unlock, lock, keyForToken, status, saveOptilens, getOptilens, reveal, saveCvApi, getCvApi, revealCvApi, saveInnovaApi, getInnovaApi, revealInnovaApi };
