...
function buildBulkTable(tableName, columns, rows) {
  const bulkTable = new sql.Table(tableName);
  bulkTable.create = false;
  for (const column of columns) {
    bulkTable.columns.add(column.name, mapZenTypeToBulkType(column), { nullable: true });
  }
...
async function bulkInsert(request, tableName, columns, rows) {
  for (const part of chunk(rows, BULK_CHUNK)) {
    await request.bulk(buildBulkTable(tableName, columns, part));
  }
}
...
>>>>>>> Stashed changes
=======
>>>>>>> 82238308abfb3c381a1105f9117bf51b7efdfcd6
}
...
async function syncFullTable(pool, connection, table, columns) {
  ...
  try {
    ...
    fetchErrors = await streamZenQuery(connection, `SELECT * FROM "${table.name}"`, async (batch) => {
      await bulkInsert(request, table.name, columns, batch);
      pulled += batch.length;
...
async function syncIncrementalTable(pool, connection, table, columns, pkColumns, { full = false } = {}) {
  ...
  const pullAll = full || !watermark;

  const query = pullAll
    ? `SELECT * FROM "${table.name}"`
    : `SELECT * FROM "${table.name}" WHERE "${WATERMARK_COLUMN}" >= ${formatZenTimestamp(new Date(watermark.getTime() - OVERLAP_MS))}`;
<<<<<<< HEAD
>>>>>>> Stashed changes
=======
>>>>>>> 82238308abfb3c381a1105f9117bf51b7efdfcd6
  ...
  const fetchErrors = await streamZenQuery(connection, query, async (batch) => {
  ...
  const reconcileDeletes = pullAll && fetchErrors.length === 0;
