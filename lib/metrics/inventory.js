...
function collectExceptions(lensItems, miscItems, overrides) {
  ...
  const consider = (item, kind) => {
    if (item.onHand < 0) {
      ...
      (sup ? suppressed : negative).push({ ...item, itemKind: kind, skuR: kind === "misc" ? item.sku : item.skuR, kind, alertKind: "negative_qty" });
    }
    if (item.onHand !== 0 && item.cost <= 0) {
      const sup = isSuppressed(overrides, kind, item.key, "zero_cost", item);
      (sup ? suppressed : zeroCost).push({ ...item, itemKind: kind, skuR: kind === "misc" ? item.sku : item.skuR, kind, alertKind: "zero_cost" });
    }
  };
...
// identifiers and per-eye quantities visible on every lens row. Misc rows put
// their single SKU in the OPC R / SKU column, rather than rendering a dash.
const EXCEPTION_COLUMNS = [
  { key: "kind", label: "Type", format: "text" },
  { key: "label", label: "Item", format: "text" },
  { key: "skuR", label: "OPC R / SKU", format: "text" },
  { key: "skuL", label: "OPC L", format: "text" },
  { key: "onHandR", label: "On hand R", format: "int", align: "right" },
  { key: "onHandL", label: "On hand L", format: "int", align: "right" },
...
function inventorySummary(items) {
  return [
    ...
    { label: "On hand", value: items.reduce((total, item) => total + item.onHand, 0), format: "int" },
    { label: "Stock value", value: items.reduce((total, item) => total + item.stockValue, 0), format: "money" },
    { label: "Units moved", value: items.reduce((total, item) => total + item.units, 0), format: "int" }
  ];
}
...
function lensProperties(item, thresholds) {
  return detailRows([
    ["Type", "Lens"],
    ["Tracked key", item.key],
    ["Manufacturer", item.manufacturer],
    ["Material", item.materialName],
    ["Material form", item.mfTypeName],
    ["Lens type", item.lensTypeName],
    ["Option", item.optionName],
    ["Item name", item.itemName],
    ["OPC R", item.skuR],
    ["OPC L", item.skuL],
    ["On hand R", item.onHandR],
    ["On hand L", item.onHandL],
    ["On hand pairs", item.onHandPairs],
    ["Total on hand", item.onHand],
    ["On order", item.onOrder],
    ["Unit cost", item.cost],
    ["Stock value", item.stockValue],
    ["Units moved", item.units],
    ["Last moved", dateValue(item.lastMoved)],
    ["Speed class", SPEED_LABELS[item.speedClass]],
    ["Cover months", item.coverMonths == null ? "—" : item.coverMonths],
    ["Treatment", item.treatment],
    ["Treatment group", item.treatmentGroup],
    ["Add power", item.addPower == null ? "—" : item.addPower],
    ["Movement window", `${thresholds.windowMonths} months`]
  ]);
}

function miscProperties(item, thresholds) {
  return detailRows([
    ["Type", "Misc"],
    ["Tracked key", item.key],
    ["SKU", item.sku],
    ...
    ["Stock value", item.stockValue],
    ["Activity basis", item.basis === "invoiced" ? "Invoiced sales" : item.basis === "purchase_proxy" ? "Purchase proxy" : "No activity"],
    ["Units moved", item.units],
    ["Units sold", item.soldUnits],
    ["Last sold", dateValue(item.lastSold)],
    ["Units received", item.receivedUnits],
    ["Last received", dateValue(item.lastReceived)],
    ["Last counted", dateValue(item.lastCounted)],
    ["Speed class", SPEED_LABELS[item.speedClass]],
    ["Cover months", item.coverMonths == null ? "—" : item.coverMonths],
    ["Movement window", `${thresholds.windowMonths} months`]
  ]);
...
const INVENTORY_DRILLS = {
  /** Every lens represented by the Items tracked headline. */
  async "inventory-tracked"() {
    const { lensItems, thresholds } = await getUniverse();
    const matched = lensItems.slice().sort((a, b) => b.stockValue - a.stockValue);
    const { rows, subtitle } = truncate(
      lensRows(matched),
      `Stocked now or moved in the last ${thresholds.windowMonths} months`
    );
    return {
      title: "Tracked lens items",
      subtitle,
      summary: inventorySummary(matched),
      columns: LENS_COLUMNS,
      rows,
      next: { kind: "inventory-item", carry: ["itemKind", "key"] }
    };
  },

  /** Every lens contributing to the Units moved headline. */
  async "inventory-moved"() {
    const { lensItems, thresholds } = await getUniverse();
    const matched = lensItems.filter((i) => i.units > 0)
      .sort((a, b) => b.units - a.units || b.stockValue - a.stockValue);
    const { rows, subtitle } = truncate(
      lensRows(matched),
      `Moved in the last ${thresholds.windowMonths} months`
    );
    return {
      title: "Lens items moved",
      subtitle,
      summary: inventorySummary(matched),
      columns: LENS_COLUMNS,
      rows,
  ...
  /**
   * One item from any inventory list. This reuses the current cached universe,
   * so row inspection stays consistent with the headline and never writes to
   * Innovations. The detail exposes the identifiers and linked classifications
   * used by the rest of the inventory analysis.
   */
  async "inventory-item"(params) {
    const itemKind = String(params.itemKind || "");
    const key = String(params.key || "");
    if (!key || !["lens", "misc"].includes(itemKind)) {
      throw Object.assign(new Error('Inventory item drill requires a valid itemKind and key.'), { statusCode: 400 });
    }

    const { lensItems, miscItems, thresholds } = await getUniverse();
    const item = (itemKind === "lens" ? lensItems : miscItems).find((candidate) => candidate.key === key);
    if (!item) {
      throw Object.assign(new Error("This inventory item is no longer in the current tracked universe."), { statusCode: 404 });
    }

    const properties = itemKind === "lens"
      ? lensProperties(item, thresholds)
      : miscProperties(item, thresholds);
    return {
      title: item.label,
      subtitle: "Current item properties used by Inventory · click Back to return to the list",
      summary: inventorySummary([item]),
      columns: [
