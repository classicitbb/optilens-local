...
// Contact delivery can require the receiver to resolve and upsert related CRM
// records. Use one longer attempt so a slow receiver is not sent duplicate
// contact batches by the normal fast-entity retry policy.
const CONTACT_FETCH_TIMEOUT_MS = Number(process.env.OPTILENS_SYNC_CONTACT_FETCH_TIMEOUT_MS ?? 120000);
const CONTACT_FETCH_RETRIES = Number(process.env.OPTILENS_SYNC_CONTACT_FETCH_RETRIES ?? 0);
...
const LENS_ALIAS_STATE_FILE = process.env.OPTILENS_LENS_ALIAS_SYNC_STATE_FILE
  || path.join(__dirname, '..', 'data', 'rx', 'cloud-alias-sync-state.json');
...
function readLensAliasSyncState(file = LENS_ALIAS_STATE_FILE) {
  try {
...
function writeLensAliasSyncState(aliases, file = LENS_ALIAS_STATE_FILE) {
  const normalized = [...new Set([...aliases].map((alias) => String(alias || '').trim()).filter(Boolean))].sort();
  const temporary = `${file}.tmp`;
  fs.mkdirSync(path.dirname(file), { recursive: true });
  fs.writeFileSync(temporary, `${JSON.stringify({ aliases: normalized, updatedAt: new Date().toISOString() }, null, 2)}\n`, 'utf8');
  fs.renameSync(temporary, file);
}

function reconcileLensAliasRecords(activeRecords, previousAliases = readLensAliasSyncState()) {
  const currentAliases = new Set(activeRecords.map((record) => String(record.alias || '').trim()).filter(Boolean));
  const syncedAt = new Date().toISOString();
  const deletedRecords = [...previousAliases]
    .filter((alias) => !currentAliases.has(alias))
    .sort()
    // The cloud schema has an active flag, so retain the record for audit and
    // make it unavailable instead of issuing an irreversible hard delete.
    .map((alias) => ({ alias, is_active: false, synced_at: syncedAt }));
  return { records: [...activeRecords, ...deletedRecords], currentAliases, deletedRecords };
}
...
const ENTITIES = {
  ...
  lens_aliases: {
    read: async () => {
      ...
      const syncedAt = new Date().toISOString();
      const activeRecords = rows.map((r) => ({
      ...
      return reconcileLensAliasRecords(activeRecords).records;
    },
    commitSnapshot: (records) => {
      writeLensAliasSyncState(records
  ...
  // Stock-lens configurator catalog. One compact row per Innovations lens
  // configuration, then one power row per LensItem; this keeps the website
  // picker independent from Website Visible / WSPL curation.
  store_lenses: {
    usesLivePool: true,
    sql: `
      SELECT DISTINCT
        CONCAT(li.MaterialGroup, ':', li.Material, ':', li.MFType, ':', li.LensType, ':', li.[Option], ':', li.Manufacturer, ':', li.Fin_Semi) AS innovations_lens_id,
        CONCAT_WS(' · ', lm.Name, mg.Name, m.Name, mf.Name, lt.Name, lo.Name) AS name,
        mg.Name AS material_group, m.Name AS material, mf.Name AS mf_type,
        lt.Name AS lens_type, lo.Name AS option_name, lm.Name AS manufacturer,
        CONCAT('Fin/Semi ', li.Fin_Semi) AS finish_type
      FROM dbo.LensItem li
      INNER JOIN dbo.LensMfgr lm ON lm.Num = li.Manufacturer
      INNER JOIN dbo.MaterialGroup mg ON mg.Groupnum = li.MaterialGroup
      INNER JOIN dbo.Materials m ON m.GroupNum = li.MaterialGroup AND m.MtrlNum = li.Material
      INNER JOIN dbo.MFTypes mf ON mf.GroupNum = li.MaterialGroup AND mf.MtrlNum = li.Material AND mf.MFNum = li.MFType
      INNER JOIN dbo.LensType lt ON lt.GroupNum = li.MaterialGroup AND lt.MatlNum = li.Material AND lt.MFnum = li.MFType AND lt.Lnum = li.LensType
      ...
      WHERE li.Flags & 2 = 0
    `,
    map: (r) => ({ ...r, innovations_lens_id: String(r.innovations_lens_id), name: String(r.name), lens_state: 'stock_lens', is_enabled: true, synced_at: new Date().toISOString() }),
  },
  store_lens_power_rows: {
    usesLivePool: true,
    sql: `
      SELECT
        CONCAT(li.MaterialGroup, ':', li.Material, ':', li.MFType, ':', li.LensType, ':', li.[Option], ':', li.Manufacturer, ':', li.Fin_Semi, ':', li.Num) AS innovations_power_row_id,
        CONCAT(li.MaterialGroup, ':', li.Material, ':', li.MFType, ':', li.LensType, ':', li.[Option], ':', li.Manufacturer, ':', li.Fin_Semi) AS innovations_lens_id,
        li.Diameter / 100.0 AS diameter, li.Base_Sphere / 100.0 AS base, li.Add_Cyl / 100.0 AS [add],
        (li.OnHand_R + li.OnHand_L + li.OnHand_P * 2) AS stock_on_hand, li.OPC_R AS right_opc, li.OPC_L AS left_opc
      FROM dbo.LensItem li
      WHERE li.Flags & 2 = 0
    `,
    map: (r) => ({ ...r, innovations_power_row_id: String(r.innovations_power_row_id), innovations_lens_id: String(r.innovations_lens_id), synced_at: new Date().toISOString() }),
  },
};

const ENTITY_ORDER = ['banks', 'customers', 'contacts', 'balances', 'order_activity', 'statements', 'statement_lines', 'lens_aliases', 'supplies', 'store_lenses', 'store_lens_power_rows'];
const DEFAULT_ENTITY_ORDER = ['banks', 'customers', 'contacts', 'balances', 'order_activity', 'statements', 'statement_lines', 'lens_aliases'];

function normalizeEntitySelection(entities = DEFAULT_ENTITY_ORDER) {
  const selected = new Set((Array.isArray(entities) ? entities : DEFAULT_ENTITY_ORDER).filter((entity) => ENTITIES[entity]));
...
function entityFetchOptions(entity) {
  if (entity === 'contacts') {
    return {
      timeoutMs: CONTACT_FETCH_TIMEOUT_MS,
      retries: CONTACT_FETCH_RETRIES,
    };
  }
...
async function sync(
  ...
  for (const entity of selectedEntities) {
    ...
    try {
      ...
      const totals = {
        read: records.length, received: 0, upserted: 0, failed: 0, batches: 0, errors: [], warnings: [],
        deactivated: entity === 'lens_aliases' ? records.filter((record) => record.is_active === false).length : 0,
      };
      ...
      // Snapshot state is an acknowledgement boundary for reconciliation
      // entities. Do not advance it after a dry run or a partial delivery:
      // otherwise a deleted source alias could be forgotten before the cloud
      // receiver has seen its inactive tombstone.
      if (totals.ok && commit && typeof ENTITIES[entity].commitSnapshot === 'function') {
        ENTITIES[entity].commitSnapshot(records);
      }
      ...
      syncLog.write('sync.entity.finished', {
        ...
        deactivated: totals.deactivated || undefined,
...
module.exports = { sync, readEntity, describeTables, functionsBase, receiverVersion, statementWindowClause, STATEMENT_WINDOW_MONTHS, ENTITIES, ENTITY_ORDER, normalizeEntitySelection, fetchNextRequest, completeRequest, runRequested, fetchWithRetry, entityFetchOptions, acquireSyncLock, LOCK_FILE, reconcileLensAliasRecords, readLensAliasSyncState, writeLensAliasSyncState, LENS_ALIAS_STATE_FILE };
