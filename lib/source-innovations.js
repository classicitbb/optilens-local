...
async function listExportCustomers(search = "") {
  ...
  const result = await request.query(`
    SELECT TOP (100)
      ...
      c.ShippingMethodID AS shippingMethodId,
      sm.ShippingMethodName AS shippingMethodName,
      c.ShipmentBin AS shipmentBin,
      CAST(1 AS bit) AS isExportCustomer,
      c.IsActive AS isActive
    FROM dbo.Customers c
    LEFT JOIN dbo.ShippingMethods sm
      ON sm.ShippingMethodID = c.ShippingMethodID
    WHERE c.ShippingMethodID = 16
      AND c.IsActive = 1
      AND (
        @search = '%%'
        OR c.AccountNumber LIKE @search
...
async function listCustomers(search = "") {
  ...
  const result = await request.query(`
    SELECT TOP (500)
      ...
      sm.ShippingMethodName AS shippingMethodName,
      c.ShipmentBin AS shipmentBin,
      CAST(CASE WHEN c.ShippingMethodID = 16 THEN 1 ELSE 0 END AS bit) AS isExportCustomer,
      c.IsActive AS isActive
    FROM dbo.Customers c
    LEFT JOIN dbo.ShippingMethods sm
      ON sm.ShippingMethodID = c.ShippingMethodID
...
async function listShipmentItems({ customerAccount, shipmentId }) {
  ...
  const result = await request.query(`
    ...
    LEFT JOIN dbo.OrderTypes ot
      ON ot.OrderType = o.OrderType
    LEFT JOIN dbo.Invoices i
      ON i.OrderID = si.OrderID
    LEFT JOIN dbo.FinARSalesJournal sj
      ON sj.InvoiceID = i.InvoiceID AND sj.OrderID = o.OrderID
...
async function searchCurrentShipmentIds(query = "") {
  ...
  const result = await pool.request()
    .input("search", `%${search}%`)
    .query(`
      SELECT TOP (200) CONVERT(nvarchar(120), s.ShipmentID) AS shipmentId
      FROM dbo.Shipments s
      LEFT JOIN dbo.Customers c ON c.CustomerID = s.ShipToID
      WHERE s.RecordDeleted = 0
        AND (
          s.Shipped = 0
          OR s.ShippedTime >= DATEADD(day, -7, CONVERT(date, GETDATE()))
        )
        AND (
          CONVERT(nvarchar(120), s.ShipmentID) LIKE @search
          OR c.AccountNumber LIKE @search
          OR c.CustomerName LIKE @search
          OR EXISTS (
            SELECT 1
            FROM dbo.ShipmentItems si
            INNER JOIN dbo.Orders o ON o.OrderID = si.OrderID
            ...
            WHERE si.ShipmentID = s.ShipmentID
              AND (
                CONVERT(nvarchar(120), i.InvoiceID) LIKE @search
                OR CONVERT(nvarchar(120), sj.RxNumber) LIKE @search
                OR CONVERT(nvarchar(300), o.PatientID) LIKE @search
              )
          )
        )
      ORDER BY s.ShipmentID DESC;
    `);
  return result.recordset.map((row) => row.shipmentId);
}
