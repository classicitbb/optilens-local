...
// Thai Optical Group appends "-RE" to a tray/job/order reference when resending
// an already-reported update; the live order is filed under the number without
// that suffix, so matching must fall back to the trimmed form.
function stripReissueSuffix(value) { return value.replace(/-RE$/i, ""); }
...
function classifyMatches(references, rows) {
  ...
  return references.map((rawReference) => {
    ...
    const candidates = byReference.get(supplierReference) || byReference.get(stripReissueSuffix(supplierReference)) || [];
...
async function matchSupplierReferencesFromZen(normalized, matchingField) {
  ...
  try {
    ...
    const queryReferences = [...new Set(normalized.map(stripReissueSuffix))];
    const rows = await connection.query(buildZenBatchMatchQuery(queryReferences, matchingField), queryReferences);
...
async function matchSupplierReferences(references, { matchingField = "customer_order_reference" } = {}) {
  ...
  try {
    ...
    queryReferences.forEach((reference, index) => request.input(`reference_${index}`, sql.NVarChar(120), reference));
    const result = await request.query(buildBatchMatchQuery(queryReferences, matchingField));
...
module.exports = { MATCH_FIELDS, ZEN_MATCH_FIELDS, buildBatchMatchQuery, buildZenBatchMatchQuery, classifyMatches, matchSupplierReferences, normalizeReference, stripReissueSuffix, zenRowToMatchRow };
