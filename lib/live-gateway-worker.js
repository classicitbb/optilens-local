...
function gatewayRequestHeaders(credentials) {
  const anonKey = String(credentials?.anonKey || '').trim();
  if (!anonKey) throw new Error('Supabase project anon key is required for the live gateway.');
  return {
...
class LiveGatewayWorker {
  ...
  async selfTest(options = {}) {
    ...
    if (creds) {
      ...
      try {
        if (!hadCreds) this.creds = { baseUrl: creds.baseUrl || '', apiKey: creds.apiKey, anonKey: creds.anonKey };
  ...
  start(creds) {
    ...
    if (!creds.anonKey) throw new Error('Supabase project anon key is required to start the live gateway.');
    this.creds = { baseUrl: creds.baseUrl || '', apiKey: creds.apiKey, anonKey: creds.anonKey };
  ...
  async post(body) {
    ...
    const response = await fetch(`${gatewayBase(this.creds.baseUrl)}/live-data-gateway`, {
      method: 'POST',
      headers: gatewayRequestHeaders(this.creds),
    ...
    if (response.status === 401) {
      // Two very different failures return 401 here and they need opposite fixes:
      //   - Supabase rejected the request BEFORE the function ran, because the
      //     function is deployed with verify_jwt on. Platform-level; the API key
      //     is irrelevant and rotating it changes nothing.
      //   - The function ran and rejected the key.
      // Supabase tags the platform-level case with an sb-error-code header
      // (UNAUTHORIZED_NO_AUTH_HEADER etc. — docs: guides/functions/error-codes),
      // which distinguishes the two with certainty rather than by guesswork.
      const sbCode = response.headers.get('sb-error-code') || '';
      const detail = parsed.error || parsed.message || parsed.detail || text.slice(0, 200) || '(empty body)';
      if (/^UNAUTHORIZED_/i.test(sbCode)) {
        throw new Error(
          `Cloud gateway returned 401 before reaching the function (sb-error-code: ${sbCode}). `
          + 'This is a DEPLOYMENT setting, not the API key — live-data-gateway is deployed '
          + 'with JWT verification enabled. Deploy it with verify_jwt = false. '
          + 'Rotating the CV API key will not help.'
        );
      }
      throw new Error(
        `Cloud gateway rejected the CV API key (401). Server said: ${detail}. `
        + 'Re-save the CV API key and confirm it carries the gateway:agent scope.'
      );
    }
...
module.exports = Object.assign(new LiveGatewayWorker(), { gatewayBase, gatewayRequestHeaders });
