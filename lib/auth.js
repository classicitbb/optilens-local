...
async function getUserAccess(userId) {
  ...
  // Temporary hardcoded permissions based on roles for the plain text store
  const permissions = [];
  if (roles.includes("admin")) {
    permissions.push("platform.admin", "users.manage", "credentials.manage", "dashboard.write", "delivery.read", "delivery.write", "pricing.read", "pricing.write");
  } else {
    permissions.push("delivery.read", "pricing.read");
  }
