...
function orderPayload(row) {
  return {
    order_id: row.order_id ?? null,
    invoice_id: row.invoice_id ?? null,
    account_number: row.account_number || row.customer_account || null,
    customer_name: row.customer_name || null,
    bill_to_account: row.bill_to_account || null,
    ship_to_account: row.ship_to_account || null,
    ...
    gen_status: row.gen_status ?? null,
    ...
    reference: row.reference || null,
    shipping_number: row.shipping_number || null,
  ...
  const result = await pool.request()
    ...
    .query(`
      SELECT TOP (100)
        ...
        c.CustomerName AS customer_name,
        CAST(bill.AccountNumber AS nvarchar(120)) AS bill_to_account,
        CAST(ship.AccountNumber AS nvarchar(120)) AS ship_to_account,
        o.OrderType AS order_type,
        ot.OrderTypeName AS order_type_name,
        o.ReceivedTime AS start_date,
        o.PromisedTime AS promised_date,
        o.ShippedTime AS shipped_date,
        o.CurrentStatusID AS status_id,
        si.StatusItemName AS status_name,
        o.CurrentStatusDate AS status_date,
        o.GenStatus AS gen_status,
        o.JobID AS job_id,
        o.CustomerTrayID AS tray_id,
        sj.RxNumber AS rx_number,
        COALESCE(NULLIF(sj.Patient, N''), NULLIF(o.PatientID, N'')) AS patient,
        COALESCE(NULLIF(sj.PoNumber, N''), NULLIF(o.CustomerPOReference, N'')) AS po_number,
        COALESCE(NULLIF(o.CustomerOrdReference, N''), NULLIF(o.BillToReference, N''), NULLIF(o.ShipToReference, N'')) AS reference,
        o.ShipMethodReference AS shipping_number,
        o.LastResultMessage AS result_message
      FROM dbo.Orders o
      INNER JOIN dbo.Customers c ON c.CustomerID = o.CustomerID
      LEFT JOIN dbo.Customers bill ON bill.CustomerID = o.BillToID
      LEFT JOIN dbo.Customers ship ON ship.CustomerID = o.ShipToID
      ...
      OUTER APPLY (
        ...
        WHERE i.OrderID = o.OrderID
          AND COALESCE(i.Status, 0) <> 99
        ORDER BY i.InvoiceTime DESC, i.InvoiceID DESC
      ) inv
      OUTER APPLY (
        SELECT TOP (1) j.RxNumber, j.Patient, j.PoNumber
        FROM dbo.FinARSalesJournal j
        WHERE j.OrderID = o.OrderID
        ORDER BY COALESCE(j.InvoiceTime, j.ReceivedTime, j.RecordCreated) DESC, j.FinARSalesJournalID DESC
      ) sj
      WHERE (
          (@customer_id IS NOT NULL AND (o.CustomerID = @customer_id OR o.BillToID = @customer_id OR o.ShipToID = @customer_id))
          OR (@account_number IS NOT NULL AND (
            c.AccountNumber = @account_number
            OR o.CustomerAccount = @account_number
            OR bill.AccountNumber = @account_number
            OR ship.AccountNumber = @account_number
          ))
        )
        AND COALESCE(o.ReceivedTime, o.RecordCreated) >= TRY_CONVERT(date, @from_date)
        AND COALESCE(o.ReceivedTime, o.RecordCreated) < DATEADD(day, 1, TRY_CONVERT(date, @to_date))
      ORDER BY COALESCE(o.ReceivedTime, o.RecordCreated) DESC, o.OrderID DESC;
    `);
...
module.exports = { OPERATIONS, dispatch, normalizeRequest, positiveInteger, dateOnly, statementPayload, statementLinePayload, orderPayload };
