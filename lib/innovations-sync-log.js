...
function summarizeEntities(entities) {
  return Object.fromEntries(Object.entries(entities || {}).map(([name, entity]) => [name, {
    ...
    durationMs: entity.durationMs != null ? Number(entity.durationMs || 0) : undefined,
    error: entity.error ? trim(entity.error) : undefined,
    // Records that synced but lost a field to a constraint clash. Surfaced in
    // the run summary so they can't hide inside an otherwise-green run.
    warnings: entity.warnings && entity.warnings.length ? entity.warnings.length : undefined,
  }]));
}
