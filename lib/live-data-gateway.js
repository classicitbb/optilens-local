...
async function getOptilensCustomerDeliveries(target, args) {
  ...
  // checklist shows). Fall back to source shipment items for those, fetched in
  // one query for the whole account and grouped by ShipmentID.
  const needsSourceItems = sessions.some(
  ...
  if (needsSourceItems) {
    try {
      ...
      const sourceItems = await listShipmentItems({ customerAccount: target.accountNumber });
      for (const item of sourceItems || []) {
        const key = String(item.shipmentId);
        if (!sourceItemsByShipment.has(key)) sourceItemsByShipment.set(key, []);
        sourceItemsByShipment.get(key).push({
      ...
      // Source lookup is best-effort — shipments still render with counts.
    }
  }
