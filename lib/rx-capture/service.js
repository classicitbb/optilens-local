...
// The employee only chooses mounting and add-ons; everything else on the
// Innovations header comes from the account picked before upload, the frame
// workflow on the draft, and the configured defaults (lab 1177, sequence 1).
function resolutionDefaults(requested, { existing, normalized, actor, mappedCustomerNumber }) {
  ...
  return {
    frameMounting: source.frameMounting,
    addonSkus: source.addonSkus,
