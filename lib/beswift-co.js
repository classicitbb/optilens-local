...
async function createAutomationJob(coApplicationId, actorUserId = null) {
  ...
  const editable = {
    ...
    items: (application.editable?.items || []).map((item) => ({ ...item, hsCode: beswiftHsCode(item.hsCode) }))
  };
...
async function getCommercialInvoicePreview(sessionId) {
  ...
  const items = composed.items.map((item, index) => {
    ...
    return {
      ...
      specification: override.specificationText || defaultSpecification,
      sourceName: item.sourceName || item.name,
      hsCode: (override.hsCodeText || item.hsCode || "").split(/\s*,\s*/).filter(Boolean).map(formatHsCode).join(", "),
...
function applyCommercialInvoiceLineOverride(base, override) {
  ...
  return { ...base, override, unitPrice, quantity, amount, value: amount };
}
