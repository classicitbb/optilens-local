    ...
    await pool.request()
      ...
      .query(`
        INSERT INTO rx_capture.orders (
          capture_order_id, status, created_by_user_id, created_by_username,
          created_by_display_name, customer_id, customer_account, customer_name,
          extracted_json, validated_json, created_at, last_updated_at
        ) VALUES (
          @capture_order_id, @status, @created_by_user_id, @created_by_username,
          @created_by_display_name, @customer_id, @customer_account, @customer_name,
          @draft_json, @draft_json, @created_at, @created_at
        );
      `);
    await recordEvent(pool, orderId, actor, "ORDER_CREATED", { manual: true, customerId: customer.id });
    return getOrder(orderId, actor);
  }
