...
// FLENS = finished lens, SLENS = semi-finished lens — both confirmed by
// Russell (2026-08-10). MISC/TINT/COAT/EDGING are the categories CVWeb's
// _price_stock_order_items assigns to supplies and add-ons (see migration
// 20260812000000_unified_order_dispatch.sql); they are NOT yet confirmed
// against Innova's own vocabulary, so confirm with Russell before relying on
// a non-lens line. SFLENS is CVWeb's spelling for semi-finished; it is
// normalised to the confirmed SLENS here rather than sent through as-is.
const SOURCE_ALIASES = { SFLENS: "SLENS" };
const VALID_SOURCES = new Set(["FLENS", "SLENS", "MISC", "TINT", "COAT", "EDGING"]);
...
function buildItems(rawItems) {
  ...
  return items.map((raw, index) => {
    ...
    const rawSource = safeText(raw?.source, 20).toUpperCase();
    const source = SOURCE_ALIASES[rawSource] || rawSource;
    if (!VALID_SOURCES.has(source)) throw httpError(`Item ${index + 1}: source must be one of ${[...VALID_SOURCES].join(", ")} — got "${raw?.source ?? ""}".`);
...
/**
 * Innova's watcher takes a released file out of the Incoming folder and
 * renames anything it cannot parse to `<name>.bad`. release() only proves the
 * copy landed — which is how earlier "successful" releases turned out to be
 * false positives (see docs/innova-stockhashref-format.md). This waits for the
 * watcher's actual verdict.
 *
 * Resolves 'accepted' (file consumed), 'rejected' (renamed .bad) or 'pending'
 * (still sitting there when the wait ran out — no verdict yet, not a failure).
 */
async function checkReleaseOutcome(filename, { timeoutMs = 30000, intervalMs = 2000 } = {}) {
  const config = loadConfig();
  if (!config.folders.incoming) throw httpError("No incoming folder configured.", 409);
  const incomingDir = path.resolve(ROOT, config.folders.incoming);
  const dropped = path.join(incomingDir, filename);
  const rejected = `${dropped}.bad`;
  const deadline = Date.now() + timeoutMs;

  for (;;) {
    if (fs.existsSync(rejected)) return "rejected";
    if (!fs.existsSync(dropped)) return "accepted";
    if (Date.now() >= deadline) return "pending";
    await new Promise((resolve) => setTimeout(resolve, intervalMs));
  }
}

module.exports = { preview, generate, release, checkReleaseOutcome, renderStockOrderText, buildStockOrder };
