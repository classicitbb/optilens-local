...
- `x_gk_order` / `x_gk_guid` — self-generated per order (not actually GK-sourced), same idea as the `.rx` pipeline, but via this module's **own** counter (see below) — not shared with `.rx`.
- `item_source` — `FLENS` = finished lens, `SLENS` = semi-finished lens (confirmed). Still caller-supplied per item, not inferred.

## Independence from the `.rx`/RXI pipeline (2026-08-10)

Russell flagged that `.rx`/RXI (patient prescription orders) and `.stockhashref` (SKU stock orders) are **completely separate order types** and told me not to conflate them. `lib/stock-order-generator.js` was refactored to be fully standalone — it no longer `require()`s `lib/rx-generator.js` at all (it did briefly, for shared utility functions like the identifier sequence and atomic file write; those are now duplicated locally). It has its own identifier counter, `data/rx/stock-sequence.json`, starting at `90000001`/`70000001` — deliberately a different range from the `.rx` pipeline's `data/rx/sequence.json` (`80000001`/`60000001`), so stock and Rx order IDs are both independent and visually distinguishable in the Incoming folder.

The only thing genuinely shared between the two is `data/rx/config.json`'s lab-level settings (`labNum`, `agentName`, `shipName`, `folders.incoming`) — because both file types are submitted to the same lab account through the same physical Incoming share, just told apart by file extension. That's a real fact about Innova's intake, not a code-level conflation.
