...
const { getBeswiftOptionIndexHints } = require("./standards-catalog");
...
const CONFIG = {
  company: {
    ...
    tin: "1111000000013",
    // The Online Payment Order's "Trader TIN" is NOT the applicant TIN above:
    // the recorded payment runs (2026-09-22) picked
    // "1000006494000 - Classic Visions Limited" there. Different register,
    // different number, so it is carried separately rather than reused.
    traderTin: "1000006494000",
    traderName: "Classic Visions Limited",
  ...
  hsCodes: {
    ...
    edgedSpectacles: { code: "90049000", description: "Spectacles, goggles and the like" }
  },
  glMapping: {
...
function resolveTrackingNumber(header, lines = []) {
  return text(header?.TrackingNumber)
    || text(header?.ShipmentReference)
    || lines.map((line) => text(line.BillToReference)).find(Boolean)
    || "";
}
...
async function prepareCoDraft(sessionId, draftInput = {}, actorUserId = null) {
  ...
  // Shipping marks are derived from account + shipment, not carried over from
  // whatever the CO form happened to be showing (e.g. a previous session's or
  // shipment's draft) — "Prepare" is explicitly a regenerate-from-Innovations
  // action, so it always resets marks to the freshly derived value. An
  // operator's manual edit still sticks via saveCoDraft (root-caused live
  // 2026-08-13, shipment 11357: a stale "NEVO 6511" from another draft kept
  // overwriting the correct marks on every re-prepare).
  editable.transport.shippingMarks = generated.payload.transport.shippingMarks;
  // Same reasoning as shippingMarks above, and for the same reason it went
  // unnoticed: the CO items grid has no editable fields of its own (readCoItems
  // just echoes back whatever was already loaded when none of its rows have a
  // data-co-field input), so mergeEditablePayload's index-merge above was
  // silently overwriting the freshly recomputed, override-corrected items with
  // whatever items happened to already be in the browser's memory — on
  // shipment 11357 that meant every re-prepare put the stale, pre-correction
  // $2,955.50 total right back after the invoice/certificate reconciliation
  // fix had already computed the correct $1,830.30.
  editable.items = generated.payload.items;
...
async function saveCoDraft(coApplicationId, draftInput = {}, actorUserId = null) {
  ...
  const { editable, warnings } = await rebuildEditableDraft(current, draftInput);
  await learnFromItems(editable.items, actorUserId);
  const pool = await getAppPool();
  const result = await pool.request()
...
async function createAutomationJob(coApplicationId, actorUserId = null) {
  ...
  // Normalize HS codes at snapshot time too: the stored editable_json is what
  // the extension actually receives, and drafts saved before beswiftHsCode()
  // existed still hold dotted codes that BeSwift's Commodity lookup can't
  // resolve (see beswiftHsCode's comment).
  const rebuilt = await rebuildEditableDraft(application);
  const editable = {
    ...rebuilt.editable,
    items: (rebuilt.editable?.items || []).map((item) => ({ ...item, hsCode: beswiftHsCode(item.hsCode) }))
  };
  const payloadSnapshot = buildAutomationJobPayload(application, editable, rebuilt.warnings);
  const pool = await getAppPool();
  const result = await pool.request()
    .input("co_application_id", coApplicationId)
...
// Auto-drive harness: the oldest still-queued, unexpired job's claim code.
// The beta extension polls this so a job created server-side starts itself,
// with no popup click — which is what makes an unattended fix/retry loop
// possible (added 2026-08-06 while root-causing the shipment 11274 run).
// Read-only and non-mutating: claiming still happens through claimAutomationJob.
async function getNextQueuedAutomationJob() {
  const pool = await getAppPool();
...
async function updateAutomationJobStatus(jobId, body = {}) {
  ...
  // "submitted" and "paid" are both terminal: a run either stops with the
  // certificate filed, or carries on through the payment order and stops when
  // the operator confirms the card window was paid. Exactly one of them is
  // reported per run, so the archive below fires once either way.
  const completed = ["filled_review", "submitted", "paid", "error", "cancelled"].includes(status);
  const pool = await getAppPool();
  const result = await pool.request()
    ...
    await pool.request()
      ...
      .query(`
        ...
        SET app_status = @app_status,
            filled_at = CASE WHEN @app_status IN (N'filled_review', N'submitted', N'paid') THEN SYSUTCDATETIME() ELSE filled_at END,
            updated_at = SYSUTCDATETIME()
        WHERE co_application_id = @co_application_id
      `);
  }
  ...
  const linesResult = await sourcePool.request()
    ...
    .query(`
      SELECT
        ...
        il.Chirality,
        il.CategoryType,
        ISNULL(prl.IsEdged, 0) AS IsPriceListEdged
      ...
      OUTER APPLY (
        SELECT MAX(CAST(priceLens.Edged AS int)) AS IsEdged
        FROM dbo.PriceRxLenses priceLens
        WHERE priceLens.PriceListID = il.PriceListID
      ) prl
...
async function buildPayloadFromShipment(session, draftInput = {}) {
  ...
  const warnings = [];
  // Certificate items are a filtered view of the exact same (override-applied)
  // commercial invoice lines, not an independent computation — otherwise the
  // certificate can file stale, pre-correction values with BeSwift while the
  // printed invoice shows the corrected ones (root-caused live 2026-08-13,
  // shipment 11357: certificate total $2,958.40 vs invoice subtotal $1,830.30).
  const composed = await buildCommercialInvoiceLines(session, lines, warnings);
  const items = composed.items
    .filter((item) => item.certificateOrder && item.certificateEligible)
    .map((item) => ({ ...item, unitCost: item.unitPrice, value: item.amount }));
  if (!items.length) {
    warnings.push("No certificate-eligible Rx invoice lines were found for this shipment.");
  }
  const certificateTotal = round(items.reduce((sum, item) => sum + Number(item.value || 0), 0), 2);
  const commercialInvoiceSubtotal = round(composed.items.reduce((sum, item) => sum + Number(item.amount || 0), 0), 2);
  const { jobCount } = composed;
  ...
  const deliveryTerms = text(customerParams?.deliveryTerms) || CONFIG.defaults.deliveryTerms;
  ...
  const trackingNumber = text(draftInput.trackingNumber) || resolveTrackingNumber(h, lines);
  if (!trackingNumber) warnings.push("Tracking/AWB is not in Innovations; enter it before queueing automation.");

  // Data-driven positional-click hints for BeSwift's non-filtering dropdowns
  // (migration 020). Carried in the payload so the extension picks by the
  // operator-editable catalog position, not a hardcoded index. Falls back to
  // the catalog constants when the DB is unreachable.
  const optionIndexHints = await getBeswiftOptionIndexHints();
  ...
  const payload = {
    ...
    // Everything the payment-order stage needs that the certificate does not.
    payment: {
      traderTin: CONFIG.company.traderTin,
      traderName: CONFIG.company.traderName,
      method: "EZpay"
    },
    importer: {
    ...
    invoiceDetails: {
      ...
      invoiceDate: shipmentClosedDate(h, session) || dateOnly(new Date())
    },
    packaging: {
      box: box.code,
      boxName: box.name,
    ...
    jobs: jobCount,
    items,
    // Surfaced so the CO draft UI can show the certificate total next to the
    // commercial invoice subtotal for a quick balance check — a gap between
    // them is expected when the shipment has non-certificate items (frames,
    // accessories), not necessarily a bug.
    certificateTotal,
    commercialInvoiceSubtotal
  };
...
async function getCommercialInvoicePreview(sessionId) {
  ...
  const portDischarge = text(editable.transport?.portOfDischarge)
    ...
    || "";
  const deliveryTerms = text(editable.transport?.deliveryTerms)
    || text(customerParams?.deliveryTerms)
    || CONFIG.defaults.deliveryTerms;
  const carrierBase = detectCarrier(h.ShippingMethodName, h.ShippingMethodID);
  const headerOverrides = await getCommercialInvoiceHeaderOverrides(sessionId);
  const carrier = text(headerOverrides.carrier) || text(editable.transport?.carrier) || CONFIG.carrierPicklist[carrierBase] || carrierBase;
  const invoiceDetails = editable.invoiceDetails || {};
  const packaging = editable.packaging || {};
  const items = composed.items.map((item, index) => {
    const commercialDescription = item.commercialDescription || item.name;
    // The commodities description belongs to the lens/item itself. PO data is
    // already shown in its own invoice header field, so including it here
    // makes a lens line less useful to customs and exceeds the intended scope.
    const defaultSpecification = item.stockOrder ? `Stock order - ${commercialDescription}` : commercialDescription;
    return {
      lineKey: item.lineKey,
      lineNumber: item.lineNumber || index + 1,
      ref: item.ref || item.invoiceNumber || item.customerReference || "",
      invoiceNumber: item.invoiceNumber || "",
      poRef: item.poRef || "",
      catalogName: item.name,
      specification: commercialDescription || defaultSpecification,
      sourceName: item.sourceName || item.name,
      hsCode: (item.hsCode || "").split(/\s*,\s*/).filter(Boolean).map(formatHsCode).join(", "),
      hsDescription: item.hsDescription || "",
      origin: item.countryOfOrigin || CONFIG.defaults.countryOfOrigin,
      quantity: item.quantity,
      unitPrice: item.unitPrice,
      amount: item.amount,
      uom: item.uom || "",
      edged: Boolean(item.edged),
      certificateEligible: Boolean(item.certificateEligible),
      stockOrder: Boolean(item.stockOrder)
    };
  });
  const subTotal = round(items.reduce((sum, item) => sum + Number(item.amount || 0), 0), 2);
  const freight = round(invoiceDetails.freightCost ?? 62, 2);
  ...
  const declarationDefault = tariffHeadings.length
    ? tariffHeadings.map((heading) => `${heading.heading}\n${heading.hsCode}`).join("\n")
    : "RX SPECTACLE LENSES OF OTHER MATERIALS";
  const declarationOverride = text(headerOverrides.declaration) || declarationDefault;
  const contactName = [h.FirstName, h.Surname].map(text).filter(Boolean).join(" ");
  const importerAddr = [h.Line1, h.Line2, h.Locality, h.PostCode].map(text).filter(Boolean).join(", ");

  // Gross weight is entered in lbs (header override) and printed in kg; each
  // line's weight is that total divided across items in proportion to
  // quantity, so line weights always sum back to the declared gross weight.
  const grossWeightLbsOverride = text(headerOverrides.grossWeightLbs);
  const grossWeightKg = round(Number(packaging.actualGrossKg || 0), 3)
    || (grossWeightLbsOverride ? lbsToKg(numberFromText(grossWeightLbsOverride, 0)) : 0);
  const totalQuantity = items.reduce((sum, item) => sum + Number(item.quantity || 0), 0);
  const unitWeightKg = totalQuantity > 0 ? grossWeightKg / totalQuantity : 0;
  const itemsWithWeight = items.map((item) => ({
    ...item,
    weightKg: round(unitWeightKg * Number(item.quantity || 0), 3)
  }));

  const customerOrderNoDefault = contactName;
  const poNumbersDefault = [...composed.poRefs][0] || "";

  return {
    generatedAt: new Date().toISOString(),
    shipmentSessionId: session.shipment_session_id,
    customerAccount: text(h.AccountNumber),
    shipmentId: Number(shipmentId),
    invoiceNo: shipmentId,
    invoiceDate: shipmentClosedDate(h, session) || dateOnly(new Date()),
    customerOrderNo: headerOverrides.customerOrderNo ?? customerOrderNoDefault,
    poNumbers: headerOverrides.poNumbers ?? poNumbersDefault,
    seller: {
    ...
    tariffHeadings,
    declarationOverride,
    declarationText: declarationDefault,
    declarationHsCode: items.find((item) => item.hsCode)?.hsCode || formatHsCode(CONFIG.hsCodes.lensOther.code),
    currency: CONFIG.defaults.currency,
    transport: {
      portOfLoading: text(headerOverrides.portOfLoading) || CONFIG.defaults.portOfLoading,
      countryOfDestination: destCountry,
      mode: CONFIG.defaults.modeOfTransport,
      carrier,
      trackingNumber: text(editable.transport?.trackingNumber) || resolveTrackingNumber(h, lines),
      marksAndNumbers: text(headerOverrides.marksAndNumbers) || text(editable.transport?.shippingMarks) || CONFIG.defaults.shippingMarksTemplate
        .replace("{ACCOUNT}", text(h.AccountNumber))
        .replace("{SHIPMENTID}", shipmentId)
    },
    packaging: {
      numberOfPackages: Number(packaging.numberOfPackages || 1),
      packageType: text(headerOverrides.packageType) || packaging.packageType || CONFIG.defaults.packageType,
      grossWeight: grossWeightKg ? `${grossWeightKg} kg` : "",
      grossWeightLbs: grossWeightKg ? String(round(grossWeightKg / 0.45359237, 3)) : "",
      cube: invoiceDetails.cubeQuantity || ""
    },
    items: itemsWithWeight,
    stockOrderOnly: itemsWithWeight.length > 0 && itemsWithWeight.every((item) => item.stockOrder),
      ...
      .query(`
        ...
        WHEN NOT MATCHED THEN
          ...
          VALUES (
            @shipment_session_id, @line_key, @line_number_text, @ref_text,
            @specification_text, @hs_code_text, @origin_text, @quantity_text,
            @unit_price_text, NULL, @actor_user_id, @actor_user_id
          );
      `);
  }
...
// Header-level print overrides for the commercial invoice preview (customer
// order no, PO numbers, carrier, marks & numbers, port of loading, package
// type, declaration, gross weight in lbs). Deliberately separate from
// delivery.co_applications.editable_json: these only affect what prints on
// this invoice and never touch the BeSwift portal-filing payload, which
// keeps its own values (tracking number, delivery terms, freight/packing/
// insurance/other costs, cube, box/packages) sourced exactly as before via
// prepareCoDraft/saveCoDraft.
const HEADER_OVERRIDE_FIELDS = {
  customerOrderNo: "customer_order_no_text",
  poNumbers: "po_numbers_text",
  carrier: "carrier_text",
  marksAndNumbers: "marks_and_numbers_text",
  portOfLoading: "port_of_loading_text",
  packageType: "package_type_text",
  declaration: "declaration_text",
  grossWeightLbs: "gross_weight_lbs_text"
};

async function getCommercialInvoiceHeaderOverrides(sessionId) {
  const pool = await getAppPool();
  const result = await pool.request()
    .input("shipment_session_id", sessionId)
    .query(`
      SELECT
        customer_order_no_text AS customerOrderNo,
        po_numbers_text AS poNumbers,
        carrier_text AS carrier,
        marks_and_numbers_text AS marksAndNumbers,
        port_of_loading_text AS portOfLoading,
        package_type_text AS packageType,
        declaration_text AS declaration,
        gross_weight_lbs_text AS grossWeightLbs
      FROM delivery.commercial_invoice_header_overrides
      WHERE shipment_session_id = @shipment_session_id
    `);
  return result.recordset[0] || {};
}

async function saveCommercialInvoiceHeaderOverrides(sessionId, fields = {}, actorUserId = null) {
  await getShipmentSession(sessionId);
  const pool = await getAppPool();
  const request = pool.request().input("shipment_session_id", sessionId).input("actor_user_id", actorUserId);
  for (const [key, column] of Object.entries(HEADER_OVERRIDE_FIELDS)) {
    // Store the value as typed, including "" — once a header row exists for a
    // shipment, an empty override means "intentionally blank" (e.g. customer
    // order no) and must not fall back to the computed default. Only a
    // missing row (no save has ever happened) means "no override yet".
    request.input(column, text(fields[key]));
  }
  await request.query(`
    MERGE delivery.commercial_invoice_header_overrides AS target
    USING (SELECT @shipment_session_id AS shipment_session_id) AS src
    ON target.shipment_session_id = src.shipment_session_id
    WHEN MATCHED THEN
      UPDATE SET
        customer_order_no_text = @customer_order_no_text,
        po_numbers_text = @po_numbers_text,
        carrier_text = @carrier_text,
        marks_and_numbers_text = @marks_and_numbers_text,
        port_of_loading_text = @port_of_loading_text,
        package_type_text = @package_type_text,
        declaration_text = @declaration_text,
        gross_weight_lbs_text = @gross_weight_lbs_text,
        updated_by_user_id = @actor_user_id,
        updated_at = SYSUTCDATETIME()
    WHEN NOT MATCHED THEN
      INSERT (
        shipment_session_id, customer_order_no_text, po_numbers_text, carrier_text,
        marks_and_numbers_text, port_of_loading_text, package_type_text,
        declaration_text, gross_weight_lbs_text, created_by_user_id, updated_by_user_id
      )
      VALUES (
        @shipment_session_id, @customer_order_no_text, @po_numbers_text, @carrier_text,
        @marks_and_numbers_text, @port_of_loading_text, @package_type_text,
        @declaration_text, @gross_weight_lbs_text, @actor_user_id, @actor_user_id
      );
  `);
  return getCommercialInvoiceHeaderOverrides(sessionId);
}

function lbsToKg(lbs) {
  return round(Number(lbs || 0) * 0.45359237, 2);
}

const LENS_BASE_CATEGORY = 1;
// CategoryType 9 holds billable lab extras (for example H1-WRAP FRAME).
// They belong to the related lens line, not outside the commercial invoice.
const LENS_ADDON_CATEGORIES = new Set([4, 6, 7, 9, 10, 11, 13]);
...
const EXCLUDED_CATEGORIES = new Set([17]);

function isCommercialInvoiceLensPart(category) {
  const value = Number(category);
  return value === LENS_BASE_CATEGORY || value === LENS_STOCK_CATEGORY || LENS_ADDON_CATEGORIES.has(value);
}

function isEdgingInvoiceLine(row) {
  // Innovations records edging as either a price-list flag on the lens or a
  // separately billed invoice line. The latter includes ordinary charges and
  // EDGINVTRIG/EDGE2FIT trigger rows, so detect the invoice evidence rather
  // than relying on one SKU spelling.
  return row?.IsPriceListEdged === true
    || Number(row?.IsPriceListEdged) === 1
    || /\bedg(?:e|ed|ing)/i.test(text(row?.Description))
    || /^EDG/i.test(text(row?.SKU));
}

function isEdgedCommercialInvoiceOrder(lines) {
  return (lines || []).some(isEdgingInvoiceLine);
}
...
function commercialInvoiceHsCode(item, entry, defaults) {
  // An edged invoice describes finished spectacles, not bare lenses. This is
  // shipment-specific source evidence and deliberately wins over a saved
  // generic lens catalog code; uncut items retain their catalog/default code.
  if (item.category === "lens" && item.edged) return CONFIG.hsCodes.edgedSpectacles.code;
  return entry?.hsCode || defaults.hsCode;
}
...
function isStockOrFulfillmentOrder(row) {
  const orderTypeName = text(row.OrderTypeName);
  // The source order type is authoritative. Patient text is only retained as
  // a legacy fallback for source records that genuinely do not carry a type.
  return orderTypeName
    ? /stock|fulfil(?:l)?ment/i.test(orderTypeName)
    : text(row.PatientID).toUpperCase() === "STOCK ORDER";
}

async function composeShipmentItems(lines, warnings) {
  ...
  for (const row of lines) {
    ...
    if (text(row.CustomerPOReference)) poRefs.add(text(row.CustomerPOReference));
  ...
  for (const [, orderLines] of byOrder) {
    ...
    const certificateOrder = isCertificateOrder(first);
    const stockOrder = orderLines.some(isStockOrFulfillmentOrder);
    const edged = isEdgedCommercialInvoiceOrder(orderLines);
    const lensParts = [];
    const frameItems = [];
    const otherItems = [];
    ...
    for (const row of orderLines) {
      ...
      if (isCommercialInvoiceLensPart(category)) {
    ...
    const poRef = text(first.CustomerPOReference);
    ...
    if (lensParts.length) {
      ...
      composed.push({
        orderKey: String(first.OrderID || ref || invoiceNumber),
  ...
  if (!composed.length) {
    warnings.push("No billable invoice lines were found to build commercial invoice items from.");
  }
  ...
  const enriched = composed.map((item) => {
    ...
    return {
      ...
      hsCode: beswiftHsCode(commercialInvoiceHsCode(item, entry, defaults)),
      hsDescription: entry?.hsDescription || defaults.hsDescription,
      // Fall back to sourceName (the un-abbreviated, joined base invoice
      // description) rather than item.name, which shortCommodityName() has
      // already truncated for catalog-matching purposes \u2014 a catalog entry
      // without a curated commercialDescription should still show the full
      // lens configuration on the invoice/certificate, not "Resin 1"
      // (root-caused live 2026-08-13, shipment 11357).
      commercialDescription: entry?.commercialDescription || item.sourceName || item.name,
      quantity: item.qty,
  ...
  // Each entry keeps a stable per-order-line key (name/price/HS/origin + this
  // order's own ref) \u2014 the same shape commercial-invoice line overrides were
  // always saved under. Grouping into display lines happens in
  // buildCommercialInvoiceLines, AFTER those overrides are applied, so a
  // correction saved against one order still resolves after composition
  // changes, and so two orders only ever merge once their (possibly
  // corrected) price genuinely matches.
  // A stock/fulfillment order is one commercial-invoice commodity even when it
  // contains many source lines. Its tariff column retains every classified HS
  // code rather than replacing the order with a placeholder sentence.
  const stockOrders = new Map();
  const normalEntries = [];
  for (const entry of enriched) {
    if (!entry.stockOrder) { normalEntries.push(entry); continue; }
    const key = entry.orderKey || entry.ref || entry.invoiceNumber;
    if (!stockOrders.has(key)) stockOrders.set(key, { ...entry, name: "Stock order", displayName: "Stock order", sourceName: "Stock order", quantity: 1, unitCost: 0, value: 0, weightKg: 0, hsCodes: new Set(), hsDescriptions: new Set() });
    const stock = stockOrders.get(key);
    stock.unitCost = round(stock.unitCost + Number(entry.unitCost || 0) * Number(entry.quantity || 0), 2);
    stock.value = stock.unitCost;
    stock.weightKg = round(stock.weightKg + Number(entry.weightKg || 0), 3);
    if (entry.hsCode) stock.hsCodes.add(entry.hsCode);
    if (entry.hsDescription) stock.hsDescriptions.add(entry.hsDescription);
  }
  const stockEntries = [...stockOrders.values()].map((stock) => ({ ...stock, hsCode: [...stock.hsCodes].join(", "), hsDescription: [...stock.hsDescriptions].join(" / "), commercialDescription: "Stock order" }));
  const entries = [...normalEntries, ...stockEntries].map((entry) => {
    const refLabel = entry.ref || entry.invoiceNumber || entry.customerReference || "";
    const keyParts = [entry.displayName, entry.unitPrice.toFixed(2), entry.hsCode, entry.countryOfOrigin, entry.stockOrder ? entry.orderKey : refLabel];
    return { ...entry, lineKey: commercialInvoiceLineKey(keyParts) };
  });

  return { entries, freightTotal, jobCount: byOrder.size, orderRefs, poRefs, invoiceNos };
}

function applyCommercialInvoiceLineOverride(base, override) {
  if (!override) return base;
  const unitPrice = round(numberFromText(override.unitPriceText, base.unitPrice), 2);
  const quantity = numberFromText(override.quantityText, base.quantity);
  const amount = round(unitPrice * quantity, 2);
  // Edging establishes a finished-spectacles tariff from the source invoice.
  // It remains authoritative even when an older manual HS override exists.
  const hsCode = base.edged
    ? CONFIG.hsCodes.edgedSpectacles.code
    : text(override.hsCodeText) || base.hsCode;
  return {
    ...base,
    override,
    lineNumber: text(override.lineNumberText) || base.lineNumber,
    ref: text(override.refText) || base.ref,
    commercialDescription: text(override.specificationText) || base.commercialDescription,
    hsCode,
    countryOfOrigin: text(override.originText) || base.countryOfOrigin,
    unitPrice,
    unitCost: unitPrice,
    quantity,
    amount,
...
// Groups same commodity/price/HS/origin lines together regardless of which
// order or invoice reference they came from \u2014 one partition shared by both
// the printed commercial invoice and the BeSwift certificate (see
// buildPayloadFromShipment below), so the two always foot to the same totals.
// Overrides are applied in two passes: first against each individual order
// line (the key a correction would have been saved under before grouping
// existed, so old corrections keep working), then again against the merged
// line itself (the key a *new* edit on the grouped row is saved under) \u2014
// previously the certificate grouped across refs while the invoice kept every
// ref apart, and the certificate was computed independently of any saved
// corrections at all, so the two documents disagreed on both line count and
// value (root-caused live 2026-08-13, shipment 11357). `refs` carries every
// merged order's reference so the grouping stays traceable instead of
// silently dropping references.
async function buildCommercialInvoiceLines(session, lines, warnings) {
  const composed = await composeShipmentItems(lines, warnings);
  const overrides = await getCommercialInvoiceLineOverrides(session.shipment_session_id);

  const adjusted = composed.entries.map((entry) => applyCommercialInvoiceLineOverride(
    { ...entry, unitPrice: entry.unitCost, quantity: entry.quantity, amount: entry.value },
    overrides.get(entry.lineKey)
  ));

  const grouped = new Map();
  for (const entry of adjusted) {
    const keyParts = [entry.displayName, entry.unitPrice.toFixed(2), entry.hsCode, entry.countryOfOrigin, entry.stockOrder ? entry.orderKey : ""];
    const key = keyParts.join("\u241F");
    if (!grouped.has(key)) {
      grouped.set(key, {
        ...entry,
        lineKey: commercialInvoiceLineKey(keyParts),
    ...
    target.amount = round(Number(target.amount || 0) + Number(entry.amount || 0), 2);
    target.value = target.amount;
    // A merged line only still qualifies as a certificate line once every
    // order folded into it is itself a certificate (Rx) order.
    target.certificateOrder = target.certificateOrder && entry.certificateOrder;
    target.stockOrder = target.stockOrder || entry.stockOrder;
    const refLabel = entry.ref || entry.invoiceNumber || entry.customerReference || "";
    if (refLabel && !target.refs.includes(refLabel)) target.refs.push(refLabel);
    if (entry.invoiceNumber && !target.invoiceNumbers.includes(entry.invoiceNumber)) target.invoiceNumbers.push(entry.invoiceNumber);
  }

  const items = [...grouped.values()].map((item) => ({
    ...applyCommercialInvoiceLineOverride(item, overrides.get(item.lineKey)),
    ref: item.refs.join(", "),
    invoiceNumber: item.invoiceNumbers.join(", ") || item.invoiceNumber
  }));

  return {
    items,
    freightTotal: composed.freightTotal,
    jobCount: composed.jobCount,
    orderRefs: composed.orderRefs,
    poRefs: composed.poRefs,
    invoiceNos: composed.invoiceNos
  };
}
...
// BeSwift's Commodity lookup only resolves bare-digit HS codes. The learned
// catalog holds both shapes ("90015000" and "9001.50.00.000") because operators
// enter them either way; the dotted form silently fails in the portal — the
// field shows "No data found" and, since Unit of Measure options are derived
// from the resolved commodity, Save then fails with "The Unit of Measure field
// is required" (root-caused live 2026-08-06, shipment 11274). Normalize to
// digits here so the extension receives a resolvable code; BeSwift's own
// classification is 8-digit, so longer padded codes are trimmed to 8.
function beswiftHsCode(value) {
  const digits = String(value || "").replace(/\D/g, "");
  if (!digits) return text(value);
  if (digits.length <= 8) return digits;
  // Trailing zeros beyond HS8 are local padding (e.g. 9001.50.00.000);
  // drop them rather than truncating a genuinely longer national code.
  const trimmed = digits.replace(/0+$/, "");
  return trimmed.length <= 8 ? digits.slice(0, 8) : digits;
}
...
function buildAutomationJobPayload(application, editable, warnings) {
  return {
...
async function rebuildEditableDraft(application, draftInput = {}) {
  const session = await getShipmentSession(application.shipmentSessionId);
  // Invoice-line overrides live apart from the draft form. Recompose from the
  // shipment every time so both an explicit Save and a direct Queue request
  // snapshot the certificate rows now shown to the operator.
  const sourceInput = {
    ...application.editable,
    ...draftInput,
    boxCode: draftInput.boxCode ?? application.editable?.packaging?.box
  };
  const generated = await buildPayloadFromShipment(session, sourceInput);
  const editable = mergeEditablePayload(generated.payload, {
    ...application.editable,
    ...draftInput,
    items: undefined
  });
  editable.items = generated.payload.items;
  return { editable, warnings: [...generated.warnings, ...validateEditablePayload(editable)] };
}
...
function getBeSwiftPortalCredentials(environment) {
  ...
  // Ordered preference, one matcher at a time. The old single call passed
  // ["beswift production", "beswift"] together, and findVaultEntry ORs the
  // matchers per entry while scanning in array order — so with a production
  // entry that wasn't named "BeSwift Production" (it was literally "New
  // entry"), the generic "beswift" matcher landed on BeSwift *Training* and
  // production jobs silently ran against the training portal with training
  // credentials (root-caused live, 2026-08-06, shipment 11274).
  const entry = findVaultEntry("Web Portals", [env === "production" ? "beswift production" : "beswift training"])
    || findVaultEntry("Web Portals", ["beswift"]);
  const fields = fieldMap(entry || {});
  // Guard: never hand production jobs a training entry (or vice versa) when
  // the entry itself declares its environment.
  if (fields.environment && normalizeEnvironment(fields.environment) !== env) {
    const error = new Error(`BeSwift ${env} portal credentials are not configured — the closest vault entry ("${entry?.name}") is for the ${normalizeEnvironment(fields.environment)} environment. Add a "BeSwift ${env === "production" ? "Production" : "Training"}" entry in Web Portals.`);
    error.statusCode = 409;
    throw error;
  }
  // "value" included because vault entries created via the generic editor
  // store the URL under a field literally labelled "Value".
  const defaultUrl = env === "production" ? "https://beswift.gov.bb/" : "https://sso.training.beswift.gov.bb/";
  const url = fields.url || fields.portalurl || fields.baseurl || fields.value || defaultUrl;
  ...
  return { environment: env, url, username, password };
}
...
// Production is the default environment: the form always submits an explicit
// value, so an absent one means a caller that never chose, and those should
// land on the portal the operators actually file against.
function normalizeEnvironment(value) {
  return String(value || "production").toLowerCase() === "training" ? "training" : "production";
}

function normalizeJobStatus(value) {
  const status = String(value || "").toLowerCase();
  return ["claimed", "logging_in", "filling", "paused", "filled_review", "submitted", "paid", "error", "cancelled"].includes(status) ? status : "filling";
}
...
// A commercial invoice is dated when its shipment closed, not by the
// individual Innovations invoices grouped onto it. Innovations' ShippedTime is
// the source closure; the app session's closed_at covers app-side closures.
function shipmentClosedDate(header, session) {
  return dateOnly(header?.ShippedTime) || dateOnly(session?.closed_at);
}
