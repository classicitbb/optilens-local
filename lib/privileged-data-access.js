...
function createPrivilegedDataAccess(deps = {}) {
  ...
  function getSource(sourceName, mode) {
    ...
    return { settings, profile, engine: sourceName.endsWith("-psql") ? "odbc" : "mssql" };
  }
  ...
  async function executeStatement(source, statement) {
    if (source.engine === "odbc") return executeOdbc(source, statement);
  ...
  async function executeOdbc(source, statement) {
    let odbc;
    try { odbc = require("odbc"); } catch { throw httpError("The ODBC module is not installed on this host.", 503); }
    let lastError;
    for (const connectionString of connectionStrings(source.profile)) {
      let connection;
      try {
        connection = await odbc.connect(connectionString);
        const rows = (await connection.query(statement)).slice(0, source.settings.maxRows);
        return { rows: normalizeRows(rows), rowCount: rows.length, recordsAffected: [] };
      } catch (error) { lastError = error; }
      finally { await connection?.close().catch(() => {}); }
    }
    throw lastError || httpError("No ODBC connection string is configured.", 503);
  }
