...
function loadCatalog() {
  const file = path.join(DATA_DIR, "lens-catalog.csv");
  let text;
  try { text = fs.readFileSync(file, "utf8"); } catch { throw httpError("RX lens catalogue is unavailable.", 503); }
  const [headerLine, ...lines] = text.replace(/^\uFEFF/, "").split(/\r?\n/).filter(Boolean);
  const headers = splitCsvLine(headerLine).map((item) => item.toLowerCase());
  const indexOf = (name) => headers.indexOf(name.toLowerCase());
  const aliasIndex = indexOf("alias");
  const materialIndex = indexOf("material");
  const mfTypeIndex = indexOf("mf type");
  const typeIndex = indexOf("lens type");
  const optionIndex = indexOf("lens option");
  const categoryIndex = indexOf("category");
  if ([aliasIndex, materialIndex, mfTypeIndex, typeIndex, optionIndex].some((index) => index < 0)) {
    throw httpError("RX lens catalogue is missing required columns.", 503);
  }
  return lines.map(splitCsvLine).map((row) => {
    const alias = String(row[aliasIndex] || "").replace(/\D/g, "");
    if (!/^\d{13}$/.test(alias)) return null;
    return {
      alias,
      materialCode: alias.slice(0, 3),
      materialDescription: safeText(row[materialIndex]),
      styleCode: alias.slice(3, 8),
      styleDescription: safeText(row[typeIndex]),
      colorCode: alias.slice(8, 13),
      colorDescription: safeText(row[optionIndex]),
      mfType: safeText(row[mfTypeIndex]) || "Single Vision",
      category: safeText(categoryIndex >= 0 ? row[categoryIndex] : "") || "All valid lenses"
    };
  }).filter(Boolean);
}

function loadItems(name) {
  const items = readJson(path.join(DATA_DIR, `${name}.json`), []);
...
function selectLens(payload, catalog, random, usedAliases) {
  ...
  let candidates = category && category !== "All valid lenses" ? catalog.filter((item) => item.category === category) : catalog;
  if (!candidates.length) throw httpError("No valid lens aliases match the selected catalogue.");
  if (mode === "fixed") {
...
function selectItems(payload, coatings, addons, frame, random) {
  ...
  if (frame.edged && !items.some((item) => item.sku === "7401")) {
    const edged = addons.find((item) => item.sku === "7401");
    if (!edged) throw httpError("The required EDGED add-on is not configured.", 503);
    items.push(edged);
  }
  return items.map((item) => ({ sku: safeText(item.sku, 80), source: safeText(item.source, 80), description: safeText(item.description, 160), quantity: String(Number(item.quantity || 1)), side: safeText(item.side || "NONE", 30), partRx: safeText(item.partRx || "Y", 10) }));
}
...
function buildOrders(payload, { reserveIdentifiers = true } = {}) {
  ...
  for (let index = 0; index < batchSize; index += 1) {
    ...
    const behaviour = LENS_BEHAVIOUR[lens.mfType] || LENS_BEHAVIOUR["Single Vision"];
...
function summarise(order) {
  return { orderId: order.identifiers.orderId, patient: order.patient.name, lens: `${order.lens.alias} · ${order.lens.materialDescription} / ${order.lens.styleDescription}`, coating: order.items.find((item) => item.source === "COAT")?.description || "None", frame: order.frame.edge, od: `${order.prescription.od.sphere} ${order.prescription.od.cylinder} × ${order.prescription.od.axis} ADD ${order.prescription.od.add}`, os: `${order.prescription.os.sphere} ${order.prescription.os.cylinder} × ${order.prescription.os.axis} ADD ${order.prescription.os.add}` };
}
...
module.exports = { getCatalog, getCoatings, getAddons, preview, generate, release, zip };
