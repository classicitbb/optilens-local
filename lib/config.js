...
function getConfig() {
  ...
  const privilegedDataAccess = {
    ...
    sources: {
      ...
      "mirror-mssql": { read: sourceMirror, write: sqlWriteProfile("OPTILENS_ADMIN_MIRROR_WRITE", sourceMirror) },
      "reporting-mssql": { read: reportingDb, write: sqlWriteProfile("OPTILENS_ADMIN_REPORTING_WRITE", reportingDb) },
      "source-psql": { read: sourcePsql, write: psqlWriteProfile(sourcePsql) }
    }
  };
...
function psqlWriteProfile(base) {
  const user = process.env.OPTILENS_ADMIN_SOURCE_PSQL_WRITE_USER || "";
  const password = process.env.OPTILENS_ADMIN_SOURCE_PSQL_WRITE_PASSWORD || "";
  if (!user || !password) return null;
  return {
    dsn: process.env.OPTILENS_ADMIN_SOURCE_PSQL_WRITE_DSN || base.dsn,
    driver: process.env.OPTILENS_ADMIN_SOURCE_PSQL_WRITE_DRIVER || base.driver,
    host: process.env.OPTILENS_ADMIN_SOURCE_PSQL_WRITE_HOST || base.host,
    port: Number(process.env.OPTILENS_ADMIN_SOURCE_PSQL_WRITE_PORT || base.port),
    database: process.env.OPTILENS_ADMIN_SOURCE_PSQL_WRITE_DATABASE || base.database,
    user,
    password
  };
}
