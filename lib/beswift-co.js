...
async function getCommercialInvoicePreview(sessionId) {
  ...
  const items = composed.items.map((item, index) => ({
    ...
    specification: item.commercialDescription || item.name,
    ...
    unitPrice: round(item.unitCost || 0, 2),
    amount: round(item.value || 0, 2),
    uom: item.uom || "",
  ...
  const hsDescriptions = [...new Set(items.map((item) => text(item.hsCode) ? "Rx spectacle lenses of other materials" : "").filter(Boolean))];
  const contactName = [h.FirstName, h.Surname].map(text).filter(Boolean).join(" ");
  const importerAddr = [h.Line1, h.Line2, h.Locality, h.PostCode].map(text).filter(Boolean).join(", ");
  ...
  return {
    ...
    deliveryTerms,
    declarationText: (hsDescriptions[0] || "Rx spectacle lenses of other materials").toUpperCase(),
...
async function composeShipmentItems(lines, warnings, options = {}) {
  ...
  for (const entry of enriched) {
    ...
    if (!grouped.has(key)) {
      grouped.set(key, { ...entry, name: entry.displayName, quantity: 0, qty: 0, value: 0 });
    }
