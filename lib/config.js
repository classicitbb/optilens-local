...
function getConfig() {
  ...
  const sourceMssqlWriter = sourceMssqlWriteProfile(sourceMssql);
...
function sourceMssqlWriteProfile(sourceMssql, environmentProfile = sqlWriteProfile("OPTILENS_ADMIN_SOURCE_MSSQL_WRITE", sourceMssql), vaultMerger = mergeSqlServerVaultEntry) {
  // Source writes deliberately use a distinct vault entry. Do not fall back to
  // the source reader: a reader that happens to have a password must never
  // become a writer merely because a writer entry was removed.
  const base = environmentProfile || {
    server: sourceMssql.server,
    database: sourceMssql.database,
    user: "",
    password: "",
    encrypt: sourceMssql.encrypt,
    trustServerCertificate: sourceMssql.trustServerCertificate
  };
  const profile = vaultMerger(base, ["source mssql writer", "source db writer"]);
  return profile.user && profile.password ? profile : null;
}
...
module.exports = { getConfig, sourceMssqlWriteProfile };
