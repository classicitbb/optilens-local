...
const ENTITIES = {
  customers: {
    sql: `
      ...
      ORDER BY CustomerID
    `,
    // Only identity fields — `type` and `pipeline_stage` have CHECK constraints
    // in the CV schema (customers_type_check etc.), so we leave them for the CRM
    // to set rather than guess allowed values. Active flag is folded into notes.
    map: (r) => ({
