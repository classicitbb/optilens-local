...
const { innovationsIncomingFolderFromVault } = require("./credential-vault");
...
function loadConfig() {
  ...
  const incomingFolder = innovationsIncomingFolderFromVault();
  if (!incomingFolder) return config;
  return {
    ...config,
    folders: { ...config.folders, incoming: incomingFolder }
  };
}
...
function selectLens(payload, catalog, random, usedAliases) {
  ...
  // Random test orders use priced stock lenses only, never customer-supplied ones.
  candidates = candidates.filter((item) => LENS_BEHAVIOUR[item.mfType] && !item.customerSupplied);
...
function nextIdentifiers() {
  const sequence = readJson(SEQUENCE_FILE, { nextOrderId: 70000000, nextGkOrder: 60000001 });
...
/**
 * Serializes a CV RxOrderForm "shape" payload (payload.shape from
 * rx-order-engine.js's buildPayload — standard-shape library outline or an
 * uploaded real tracer file, both already reduced to polar radii in mm) into
 * a real Innova trace_start/trace_end block.
 *
 * IMPORTANT: shape.radii/shape.nativeBox are the outline's own NATIVE
 * (unscaled) geometry — rx-order-engine.js only rescales to the typed A/B
 * for the on-screen SVG preview (shapeGeometry()'s ptsR/ptsL), it never
 * mutates or re-exports the scaled polar points. So this block is built
 * entirely from shape.nativeBox, deliberately ignoring whatever A/B the
 * order header carries — sending a HBOX/VBOX that doesn't match the radii
 * array would be a worse inconsistency than the header and the trace
 * disagreeing on size. This isn't a guess: sample-progressive-enclosed-
 * traced.rx does exactly this in the wild (trace HBOX=59.02/VBOX=46.90 vs
 * that same order's frame_a:56.0/frame_b:37.0 — a real, presumably
 * lab-accepted order with a native-trace-box that differs from the ordered
 * blank size).
 *
 * shape.computed.{edAxis,circ} have no native-basis equivalent in the
 * payload (rx-order-engine.js only computes them off the rescaled preview
 * geometry) — used here as the closest available approximation and flagged
 * in docs/rx-format-field-map.md. .ED itself DOES have a native value
 * (shape.nativeBox.ed, straight from the source file's own .ED/FED field)
 * and that's what's used, not the rescaled shape.computed.ed.
 *
 * Verified against real LabLink .rx exports (templates/rx-samples/): a
 * mirrored second eye is OMITTED entirely (Innova mirrors it themselves) —
 * matches sample-progressive-enclosed-traced.rx, which sends only the R
 * side. A genuinely distinct second eye gets its own TRCFMT/R= block —
 * matches sample-progressive-wrap-prism-enclosed-traced.rx. Bevel (Z=) data
 * is never available from this payload shape (the browser-side OMA parser
 * doesn't extract it either), so every block always closes with plain
 * "ZFMT=0" — that part is confirmed correct by both real samples.
 *
 * Returns "" (no trace) if the shape has no usable radii — callers must
 * treat that as "send x_standard_shape_trace:false, no block" rather than
 * emit an empty trace_start/trace_end pair.
 */
function renderTraceBlock(shape) {
  if (!shape || !shape.radii || !Array.isArray(shape.radii.R) || !shape.radii.R.length) return "";
  const fmt1 = (value) => Number(value ?? 0).toFixed(2);
  const toHundredths = (mm) => String(Math.round(Number(mm) * 100));
  const chunk15 = (values) => {
    const lines = [];
    for (let i = 0; i < values.length; i += 15) lines.push(`R=${values.slice(i, i + 15).join(";")}`);
    return lines;
  };
  const ed = shape.nativeBox?.ed ?? shape.computed?.ed ?? 0;
  const edAxisR = shape.computed?.edAxis ?? 0;
  const mirrored = shape.mirroredFrom === "right" || shape.mirroredFrom === "left";
  // Mirroring a boundary about the vertical axis flips its ED axis to (180 - axis) —
  // confirmed by sample-progressive-enclosed-traced.rx's .AX=26.72;153.28 (sums to 180).
  const edAxisL = mirrored ? (((180 - edAxisR) % 180) + 180) % 180 : edAxisR;
  const circ = shape.computed?.circ ?? 0;
  const a = shape.nativeBox?.a ?? 0;
  const b = shape.nativeBox?.b ?? 0;
  const dbl = shape.nativeBox?.dbl ?? 0;

  const sideBlock = (radiiMm, side) => [
    `TRCFMT=1;${radiiMm.length};E;${side};F`,
    ...chunk15(radiiMm.map(toHundredths)),
    "ZFMT=0"
  ];
  ...
  const lines = [
    "trace_start",
    "REQ=FIL",
    `JOB="${safeText(shape.job || "0000", 20)}"`,
    `DBL=${fmt1(dbl)}`,
    `CIRC=${fmt1(circ)};${fmt1(circ)}`,
    `HBOX=${fmt1(a)}; ${fmt1(a)}`,
    `VBOX=${fmt1(b)}; ${fmt1(b)}`,
    `.ED=${fmt1(ed)};${fmt1(ed)}`,
    `.AX=${fmt1(edAxisR)};${fmt1(edAxisL)}`,
    ...sideBlock(shape.radii.R, "R")
  ];
  if (!mirrored && Array.isArray(shape.radii.L) && shape.radii.L.length) {
    lines.push(...sideBlock(shape.radii.L, "L"));
  }
  lines.push("trace_end");
  return `${lines.join("\n")}\n`;
}
...
function renderRxText(order, config) {
  ...
  const traceBlock = order.traceBlockText ?? "";
  const values = {
    ...
    standard_shape_trace: traceBlock ? "true" : "false", trace_block: traceBlock,
...
module.exports = { getCatalog, getCoatings, getAddons, preview, generate, release, zip, renderRxText, renderTraceBlock, loadConfig, nextIdentifiers, LENS_BEHAVIOUR, atomicWrite, filenameFor, formatDate };
