...
Business Metrics now has a dedicated **OpenAI API — GPT-5.6 Luna** Chat API preset. It selects the existing OpenAI-compatible endpoint and the exact `gpt-5.6-luna` model ID; the generic OpenAI choice is retained for custom model IDs. The saved application setting now uses the OpenAI endpoint with `gpt-5.6-luna`. The Chat Completions request deliberately omits `temperature` for Luna IDs because the provider rejects non-default values, while preserving the existing `0.1` temperature for other models. Source syntax, the focused assistant suite (7/7), and `npm run check` pass. The local source fix has not been deployed or restarted, so an approved guarded application update/restart is required before sending a normal assistant question to verify the repaired request; that question is billable. Do not disable TLS verification.
