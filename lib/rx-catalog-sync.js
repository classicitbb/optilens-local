const fs = require("node:fs");
const path = require("node:path");
const { getSourcePool } = require("./db");

const ROOT = path.join(__dirname, "..");
const PRICING_ROWS_FILE = path.join(ROOT, "data", "pricelist", "catalog-rows.generated.json");
const RX_DIR = path.join(ROOT, "data", "rx");
const CATALOG_FILE = path.join(RX_DIR, "catalog.generated.json");
const ADDONS_FILE = path.join(RX_DIR, "addons.generated.json");
const COATINGS_FILE = path.join(RX_DIR, "coatings.generated.json");
const METADATA_FILE = path.join(RX_DIR, "catalog-source.json");

const normalise = (value) => String(value || "").replace(/\s+/g, " ").trim().toLowerCase();
// These are reviewed source-name equivalences, not fuzzy matching. Pricing Automation
// and Innovations use different labels for the same material in these cases.
const MATERIAL_NAME_EQUIVALENTS = new Map([
  ["cr-39 1.50", "plastic 1.50"], ["plastic 1.50", "plastic 1.50"],
  ["high index 1.67", "high index 1.67"], ["1.67 high index", "high index 1.67"], ["1.67 index", "high index 1.67"],
  ["high index 1.74", "high index 1.74"], ["1.74 high index", "high index 1.74"], ["1.74 index", "high index 1.74"],
  ["polarized", "polarized 1.50"], ["polarized 1.50", "polarized 1.50"],
  ["poly", "poly 1.59"], ["poly 1.59", "poly 1.59"], ["polycarb 1.59", "poly 1.59"],
  ["photochromic 1.50", "photochromic 1.50"], ["transitions 1.50", "photochromic 1.50"],
  ["trivex 1.53", "trivex 1.53"]
]);
const canonicalMaterial = (value) => MATERIAL_NAME_EQUIVALENTS.get(normalise(value)) || normalise(value);
const pricingKey = (material, ...parts) => [canonicalMaterial(material), ...parts.map(normalise)].join("\u0001");

function isCustomerSuppliedLens(style, option) {
  return normalise(style) === "custom lens" || normalise(option) === "custom lens";
}

function readJson(file, fallback) {
  try { return JSON.parse(fs.readFileSync(file, "utf8")); } catch { return fallback; }
}

function writeJson(file, value) {
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const temporary = `${file}.tmp`;
  fs.writeFileSync(temporary, `${JSON.stringify(value, null, 2)}\n`, { encoding: "utf8", flag: "w" });
  fs.renameSync(temporary, file);
}

function pricingKeys() {
  const rows = readJson(PRICING_ROWS_FILE, []);
  if (!Array.isArray(rows) || !rows.length) throw new Error("Pricing Automation catalogue rows are unavailable. Refresh the pricing catalogue first.");
  const values = new Map();
  for (const row of rows) {
    if (row?.active === false || !row?.material || !row?.mftype || !row?.lenstype) continue;
    const sourceKey = pricingKey(row.material, row.mftype, row.lenstype);
    const entry = values.get(sourceKey) || {
      material: String(row.material).trim(), mfType: String(row.mftype).trim(), lensType: String(row.lenstype).trim(), suppliers: new Set()
    };
    const supplier = String(row.supplier || "").trim();
    if (supplier) entry.suppliers.add(supplier);
    values.set(sourceKey, entry);
  }
  return values;
}

async function syncRxCatalog() {
  const pricing = pricingKeys();
  const pool = await getSourcePool();
    // Resolve dimensions in the same query as the alias. The dimension tables contain
    // historical duplicate keys, so independent maps can overwrite a valid description.
    const aliases = (await pool.request().query(`SELECT DISTINCT
      a.Alias AS alias, a.GroupNum AS material_group_code, mg.Name AS material_group, m.Name AS material, mf.Name AS mf_type, lt.Name AS lens_type, lo.Name AS option_name
      FROM dbo.LensAlias a
      JOIN dbo.MaterialGroup mg ON mg.Groupnum = a.GroupNum
      JOIN dbo.Materials m ON m.MtrlNum = a.MtrlNum AND m.GroupNum = a.GroupNum
      JOIN dbo.MFTypes mf ON mf.MFNum = a.MFNum AND mf.MtrlNum = a.MtrlNum AND mf.GroupNum = a.GroupNum
      JOIN dbo.LensType lt ON lt.Lnum = a.LNum AND lt.MFnum = a.MFNum AND lt.Matlnum = a.MtrlNum AND lt.Groupnum = a.GroupNum
      JOIN dbo.LensOptions lo ON lo.Num = a.OptNum AND lo.Lnum = a.LNum AND lo.MFnum = a.MFNum AND lo.Matlnum = a.MtrlNum AND lo.Groupnum = a.GroupNum`)).recordset;
    const [miscRows, miscGroups] = await Promise.all([
      pool.request().query('SELECT MiscItemID AS item_id, MiscItemGroupID AS group_id, Alias AS alias, SKU AS sku, Desc1 AS description, DefaultRxQuantity AS default_quantity FROM dbo.MiscItems').then((result) => result.recordset),
      pool.request().query('SELECT MiscItemGroupID AS group_id, MiscItemGroupPID AS parent_group_id, MiscItemGroupName AS name FROM dbo.MiscItemGroups').then((result) => result.recordset)
    ]);

    const seen = new Set();
    const records = [];

    for (const row of aliases) {
      const alias = String(row.alias || "").trim();
      if (!/^\d{13}$/.test(alias) || seen.has(alias)) continue;
      const material = String(row.material || "").trim();
      const mfType = String(row.mf_type || "").trim();
      const lensType = String(row.lens_type || "").trim();
      const lensOption = String(row.option_name || "").trim();
      const priceRow = pricing.get(pricingKey(material, mfType, lensType));
      // Customer-supplied lenses (the Innovations "Custom Lens" style) are cut
      // from lenses the customer sends in, so they carry no supplier price.
      const customerSupplied = isCustomerSuppliedLens(lensType, lensOption);
      if ((!priceRow && !customerSupplied) || !lensOption) continue;
      seen.add(alias);
      records.push({
        alias,
        materialGroupCode: String(row.material_group_code),
        materialGroup: String(row.material_group || "").trim(),
        materialCode: alias.slice(0, 3),
        materialDescription: material,
        styleCode: alias.slice(3, 8),
        styleDescription: lensType,
        colorCode: alias.slice(8, 13),
        colorDescription: lensOption,
        mfType,
        category: mfType,
        suppliers: priceRow ? [...priceRow.suppliers].sort((left, right) => left.localeCompare(right)) : [],
        pricingKey: priceRow ? `${priceRow.material} | ${priceRow.mfType} | ${priceRow.lensType}` : null,
        ...(customerSupplied ? { customerSupplied: true } : {}),
        source: "Innovations LensAlias"
      });
    }

    records.sort((left, right) => left.alias.localeCompare(right.alias));
    const matchedPricingKeys = new Set(records.filter((row) => row.pricingKey).map((row) => pricingKey(row.materialDescription, row.mfType, row.styleDescription)));
    const unmatchedPricing = [...pricing.values()].filter((row) => !matchedPricingKeys.has(pricingKey(row.material, row.mfType, row.lensType)));
    const groupsByParent = new Map();
    const groupNames = new Map(miscGroups.map((group) => [Number(group.group_id), String(group.name || "").trim()]));
    for (const group of miscGroups) {
      const parent = Number(group.parent_group_id);
      if (!Number.isInteger(parent)) continue;
      const children = groupsByParent.get(parent) || [];
      children.push(Number(group.group_id));
      groupsByParent.set(parent, children);
    }
    const coatingGroupIds = new Set([3]);
    for (const id of coatingGroupIds) for (const child of groupsByParent.get(id) || []) coatingGroupIds.add(child);
    const misc = miscRows
      .map((row) => ({
        sku: String(row.sku || row.alias || "").trim(),
        source: "MISC",
        description: String(row.description || "").trim(),
        quantity: Math.max(1, Number(row.default_quantity || 1)),
        side: "NONE",
        partRx: "Y",
        active: true,
        sourceItemId: Number(row.item_id),
        sourceAlias: String(row.alias || "").trim(),
        groupId: Number(row.group_id),
        groupName: groupNames.get(Number(row.group_id)) || "Miscellaneous"
      }))
      .filter((row) => row.sku && row.description)
      .sort((left, right) => left.description.localeCompare(right.description));

    const edgeItem = misc.find((item) => normalise(item.description) === "edge to fit") || null;
    const coatings = misc.filter((item) => coatingGroupIds.has(item.groupId)).map((item) => ({ ...item, kind: "coating" }));
    const metadata = {
      generatedAt: new Date().toISOString(),
      source: "Innovations MSSQL LensAlias / MiscItems",
      pricingSource: "data/pricelist/catalog-rows.generated.json",
      aliasesRead: aliases.length,
      validPricingAliases: records.filter((row) => row.pricingKey).length,
      customerSuppliedAliases: records.filter((row) => row.customerSupplied).length,
      coatingGroup: { id: 3, name: groupNames.get(3) || "Coatings", descendantGroupIds: [...coatingGroupIds].sort((a, b) => a - b) },
      coatingItems: coatings.length,
      pricingKeys: pricing.size,
      matchedPricingKeys: matchedPricingKeys.size,
      materialEquivalences: Object.fromEntries(MATERIAL_NAME_EQUIVALENTS),
      unmatchedPricingKeys: unmatchedPricing.map((row) => `${row.material} | ${row.mfType} | ${row.lensType}`),
      edgeItem
    };

    if (!records.length) throw new Error("No source aliases matched the active Pricing Automation catalogue; RX generation remains disabled.");
    writeJson(CATALOG_FILE, records);
    writeJson(ADDONS_FILE, edgeItem ? [edgeItem] : []);
    writeJson(COATINGS_FILE, coatings);
    writeJson(METADATA_FILE, metadata);
    return metadata;
}

async function syncRxCoatings() {
  const pool = await getSourcePool();
    const [miscRows, miscGroups] = await Promise.all([
      pool.request().query('SELECT MiscItemID AS item_id, MiscItemGroupID AS group_id, Alias AS alias, SKU AS sku, Desc1 AS description, DefaultRxQuantity AS default_quantity FROM dbo.MiscItems').then((result) => result.recordset),
      pool.request().query('SELECT MiscItemGroupID AS group_id, MiscItemGroupPID AS parent_group_id, MiscItemGroupName AS name FROM dbo.MiscItemGroups').then((result) => result.recordset)
    ]);
    const groupsByParent = new Map();
    const groupNames = new Map(miscGroups.map((group) => [Number(group.group_id), String(group.name || "").trim()]));
    for (const group of miscGroups) {
      const parent = Number(group.parent_group_id);
      if (!Number.isInteger(parent)) continue;
      const children = groupsByParent.get(parent) || [];
      children.push(Number(group.group_id));
      groupsByParent.set(parent, children);
    }
    const coatingGroupIds = new Set([3]);
    for (const id of coatingGroupIds) for (const child of groupsByParent.get(id) || []) coatingGroupIds.add(child);
    const items = miscRows.map((row) => ({
      sku: String(row.sku || row.alias || "").trim(), source: "MISC", description: String(row.description || "").trim(),
      quantity: Math.max(1, Number(row.default_quantity || 1)), side: "NONE", partRx: "Y", active: true,
      sourceItemId: Number(row.item_id), sourceAlias: String(row.alias || "").trim(), groupId: Number(row.group_id),
      groupName: groupNames.get(Number(row.group_id)) || "Miscellaneous"
    })).filter((row) => row.sku && row.description);
    const coatings = items.filter((item) => coatingGroupIds.has(item.groupId)).map((item) => ({ ...item, kind: "coating" }));
    const edgeItem = items.find((item) => normalise(item.description) === "edge to fit") || null;
    if (!coatings.length) throw new Error("No live coating options were found in the Innovations Coatings group.");
    writeJson(ADDONS_FILE, edgeItem ? [edgeItem] : []);
    writeJson(COATINGS_FILE, coatings);
    const metadata = readJson(METADATA_FILE, {});
    writeJson(METADATA_FILE, { ...metadata, generatedAt: new Date().toISOString(), coatingItems: coatings.length, edgeItem });
    return { coatingItems: coatings.length, edgeItem };
}

module.exports = { syncRxCatalog, syncRxCoatings, canonicalMaterial, isCustomerSuppliedLens, CATALOG_FILE, ADDONS_FILE, COATINGS_FILE, METADATA_FILE };
