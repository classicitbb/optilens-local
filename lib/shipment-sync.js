 ...
 *
 * app_status follows Shipped in both directions (closes AND reopens) for any
 * row this sync closed on its own. The one exception is a row a dispatcher
 * closed locally via closeShipmentSessionsBatch (lib/delivery.js) ahead of
 * Innovations catching up - those are recognizable by a non-null
 * dispatcher_id, and are left alone so a lagging Shipped bit can't undo a
...
async function upsertSession(pool, row) {
  ...
  const result = await request.query(`
    ...
    WHEN MATCHED THEN UPDATE SET
      ...
      app_status = CASE
        WHEN @app_status = N'closed' THEN N'closed'
        WHEN target.dispatcher_id IS NULL THEN N'prep'
        ELSE target.app_status
      END,
      closed_at = CASE
        WHEN @app_status = N'closed' THEN @closed_at
        WHEN target.dispatcher_id IS NULL THEN NULL
        ELSE target.closed_at
      END,
