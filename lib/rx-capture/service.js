...
function createRxCaptureService(dependencies = {}) {
  ...
  async function saveResolution(orderId, payload, actor) {
    ...
    const resolved = normalizeResolution({
      ...payload?.resolution,
      customerNumber: existing.customer_account,
  ...
  return { approveOrder, createOrder, getApprovalOrder, getOrder, listApprovalQueue, listCoatings, listOrders, processOrder, releaseOrder, reprocessOrder, saveResolution, stageOrder, submitOrder, suggestAlias, updateOrder };
}
