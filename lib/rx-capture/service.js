...
function createRxCaptureService(dependencies = {}) {
  ...
  async function listOrders(actor, limit = 30, search = "") {
    const pool = await getPool();
    // Search matches patient, customer name/account or the generated order file.
    const term = String(search || "").trim().slice(0, 100);
    const request = pool.request()
      .input("user_id", sql.UniqueIdentifier, actor.userId)
      .input("limit", sql.Int, clampLimit(limit));
    if (term) request.input("search", sql.NVarChar(210), `%${term.replace(/[!%_[]/g, "!$&")}%`);
    const result = await request.query(`
        ...
        WHERE created_by_user_id = @user_id AND status <> N'CANCELLED'${term ? `
          AND (patient_name LIKE @search ESCAPE '!' OR customer_name LIKE @search ESCAPE '!'
            OR customer_account LIKE @search ESCAPE '!' OR generated_filename LIKE @search ESCAPE '!')` : ""}
  ...
  // Drafts that were never submitted can be thrown away. The row is kept with
  // status CANCELLED for the audit trail; its temporary photos are deleted.
  async function discardOrder(orderId, actor) {
    const existing = await ownedOrder(orderId, actor);
    if (!DISCARDABLE_STATUSES.includes(existing.status)) {
      throw httpError(existing.status === "PROCESSING"
        ? "This order is still being read. Wait for it to finish before discarding it."
        : "Only unsubmitted drafts can be discarded.", 409);
    }
    const pool = await getPool();
    const result = await pool.request()
      .input("capture_order_id", sql.UniqueIdentifier, orderId)
      ...
      .query(`
        UPDATE rx_capture.orders
        SET status = N'CANCELLED', last_updated_at = SYSUTCDATETIME()
        WHERE capture_order_id = @capture_order_id AND created_by_user_id = @user_id
          AND status IN (N'NEEDS_INFO', N'READY_FOR_REVIEW', N'FAILED');
      `);
    if (!result.rowsAffected[0]) throw httpError("This order changed and can no longer be discarded. Reload it and try again.", 409);
    await recordEvent(pool, orderId, actor, "CANCELLED", { previousStatus: existing.status });
    const paths = parseStoredPaths(existing.source_image_paths_json);
    if (paths.length) {
      await deleteFiles(paths);
      await pool.request()
        .input("capture_order_id", sql.UniqueIdentifier, orderId)
        .query("UPDATE rx_capture.orders SET source_image_deleted_at = SYSUTCDATETIME() WHERE capture_order_id = @capture_order_id AND source_image_deleted_at IS NULL;");
    }
  ...
  return { approveOrder, createOrder, discardOrder, frequentCustomers, getApprovalOrder, getOrder, listApprovalQueue, listCatalog, listCoatings, listOrders, processOrder, releaseOrder, reprocessOrder, saveResolution, stageOrder, submissionAccount, submitOrder, suggestAlias, transcribeDictation, updateOrder, voiceStatus };
}
...
const LOCKED_STATUSES = ["APPROVED", "RX_GENERATED", "STAGED", "RELEASED", "CANCELLED"];
const DISCARDABLE_STATUSES = ["NEEDS_INFO", "READY_FOR_REVIEW", "FAILED"];

function assertEditable(order) {
  if (order.status === "CANCELLED") throw httpError("This draft was discarded. Create a new capture.", 409);
