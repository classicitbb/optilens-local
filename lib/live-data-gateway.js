...
function normalizeRequest(request) {
  ...
  if (request.operation === 'innovations.customer_invoice' && !positiveInteger(args.invoice_id)) {
    throw Object.assign(new Error('invoice_id must be a positive integer.'), { code: 'invalid_invoice_id' });
  }
...
const CUSTOMER_ORDER_STATUS_QUERY = `
  ...
  OUTER APPLY (
    SELECT TOP (1) i.InvoiceID, i.Total
    FROM dbo.Invoices i
    WHERE i.OrderID = matching_orders.order_id
...
async function getInnovationsCustomerInvoice(target, invoiceId) {
  const pool = await getSourcePool();
  const result = await pool.request()
    ...
    .query(`
      SELECT TOP (1)
        i.InvoiceID AS id,
        i.InvoiceTime AS invoice_date,
        o.PatientID AS patient,
        i.Total AS total
      FROM dbo.Invoices i
      INNER JOIN dbo.Orders o ON o.OrderID = i.OrderID
      LEFT JOIN dbo.Customers bill_to ON bill_to.CustomerID = o.BillToID
      WHERE i.InvoiceID = @invoice_id
        AND COALESCE(i.Status, 0) <> 99
        AND (
          (@order_lookup = 'bill_to' AND @account_number IS NOT NULL AND bill_to.AccountNumber = @account_number)
          OR (@order_lookup <> 'bill_to' AND @customer_id IS NOT NULL AND o.CustomerID = @customer_id)
          OR (@order_lookup <> 'bill_to' AND @customer_id IS NULL AND @account_number IS NOT NULL AND o.CustomerAccount = @account_number)
          OR EXISTS (
            SELECT 1
            FROM dbo.FinARStatementItems si
            INNER JOIN dbo.FinARStatements ss ON ss.FinARStatementID = si.FinARStatementID
            INNER JOIN dbo.Customers c ON c.CustomerID = ss.CustomerID
            WHERE si.InvoiceID = i.InvoiceID
              AND si.HideFromStatement = 0
              AND ss.Void = 0
              AND ((@customer_id IS NOT NULL AND c.CustomerID = @customer_id)
                OR (@customer_id IS NULL AND @account_number IS NOT NULL AND c.AccountNumber = @account_number))
          )
      ...
      SELECT
        ...
        il.InvoicePrice AS unit_price,
        CAST(COALESCE(il.Quantity, 0) * COALESCE(il.InvoicePrice, 0) AS decimal(18, 2)) AS amount
      FROM dbo.InvoiceLines il
      WHERE il.InvoiceID = @invoice_id
        AND COALESCE(il.Suppressed, 0) = 0
        AND EXISTS (
          SELECT 1
          FROM dbo.Invoices i
          INNER JOIN dbo.Orders o ON o.OrderID = i.OrderID
          ...
          WHERE i.InvoiceID = il.InvoiceID
            ...
            AND (
              ...
              OR (@order_lookup <> 'bill_to' AND @customer_id IS NULL AND @account_number IS NOT NULL AND o.CustomerAccount = @account_number)
              OR EXISTS (
                SELECT 1 FROM dbo.FinARStatementItems si
                INNER JOIN dbo.FinARStatements ss ON ss.FinARStatementID = si.FinARStatementID
                INNER JOIN dbo.Customers c ON c.CustomerID = ss.CustomerID
                WHERE si.InvoiceID = i.InvoiceID
                  AND si.HideFromStatement = 0 AND ss.Void = 0
                  AND ((@customer_id IS NOT NULL AND c.CustomerID = @customer_id)
                    OR (@customer_id IS NULL AND @account_number IS NOT NULL AND c.AccountNumber = @account_number))
              )
            )
  ...
  if (!invoice) throw Object.assign(new Error('Invoice was not found for this customer.'), { code: 'invoice_not_found' });
  return {
    invoice: { ...invoice, total: nullableNumber(invoice.total) },
    lines: (result.recordsets[1] || []).map((line) => ({
      id: line.id ?? null,
      description: line.description || null,
      quantity: nullableNumber(line.quantity),
      unit_price: nullableNumber(line.unit_price),
...
async function getOptilensCustomerDeliveries(target, args) {
  ...
  for (const item of itemsResult.recordset || []) {
    ...
    itemsBySession.get(key).push({
      ...
      invoice_id: item.invoice_number ?? null,
      amount: nullableNumber(item.price),
      quantity: null,
    ...
    await Promise.all(uniqueSourceShipmentIds.map(async (shipmentId) => {
      try {
        ...
        sourceItemsByShipment.set(String(shipmentId), (sourceItems || []).map((item) => ({
          ...
          invoice_id: item.invoiceNumber ?? null,
          amount: nullableNumber(item.price),
          quantity: null,
...
async function dispatch(request) {
  ...
  if (normalized.operation === 'innovations.customer_invoice') {
    return getInnovationsCustomerInvoice(normalized.target, positiveInteger(normalized.args.invoice_id));
  }
