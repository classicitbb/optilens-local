...
const { resolveLensAlias } = require("./alias-resolver");
...
function createRxCaptureService(dependencies = {}) {
  ...
  async function saveResolution(orderId, payload, actor) {
    ...
    const resolved = normalizeResolution({
      ...payload?.resolution,
      coatingSku: payload?.resolution?.coatingSku || normalized?.lensRequest?.coatingSku || null,
      lensAlias: payload?.resolution?.lensAlias || normalized?.lensRequest?.catalogAlias || "",
      shipName: payload?.resolution?.shipName || existing.customer_name,
  ...
  async function suggestAlias(orderId, actor) {
    ...
    return resolveLensAlias(normalized, getCatalog());
  }
  ...
  async function listCatalog() {
    return getCatalog()
      ...
      .map((item) => ({
        ...
        design: String(item.styleDescription || ""),
  ...
  async function submissionAccount(orderId, actor) {
    ...
    return { customerAccount: existing.customer_account, customerNumber: result.recordset[0]?.innovations_customer_number || null };
  }
