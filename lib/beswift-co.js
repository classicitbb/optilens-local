...
const CONFIG = {
  company: {
    ...
    country: "Barbados",
    contacts: [
      { name: "Randall Hunte", mobile: "+12464334928", email: "info@classicvisions.net" },
  ...
  defaults: {
    ...
    originCriterion: "L (46.9% per company records)",
    packageType: "Box, fibreboard",
...
async function buildPayloadFromShipment(session, draftInput = {}) {
  ...
  const warnings = [];
  const aggregate = aggregateLines(linesResult.recordset, warnings);
  const jobCount = aggregate.orderIds.size;
  const items = buildItems(aggregate, jobCount, warnings);
  ...
  const portDischarge = CONFIG.ports[destCountry] || "";
  const carrierBase = CONFIG.carriersByMethod[String(h.ShippingMethodID)] || "DHL";
  ...
  const payload = {
    ...
    transport: {
      ...
      carrier,
      shippingMarks: marks,
      shippingDate: dateOnly(h.ShippedTime) || "",
      deliveryTerms: CONFIG.defaults.deliveryTerms,
      trackingNumber: text(h.TrackingNumber)
    },
    invoiceDetails: {
      currency: CONFIG.defaults.currency,
      customerOrderNo: [...aggregate.orderRefs].slice(0, 5).join(", "),
      cubeQuantity: box.cubeM3,
      cubeUnit: CONFIG.defaults.cubeUnit,
      packingCost: 0,
      freightCost: round(aggregate.buckets.freight, 2),
      insuranceCost: 0,
      otherCost: 0,
      invoiceNumbers: [...aggregate.invoiceNos].join(", "),
      invoiceDate: [...aggregate.invoiceDates].sort().at(-1) || ""
    },
    packaging: {
      box: box.code,
      boxName: box.name,
...
function aggregateLines(lines, warnings) {
  const buckets = { lensOther: 0, lensGlass: 0, extras: 0, frames: 0, accessories: 0, freight: 0 };
  const qty = { lensOther: 0, lensGlass: 0, frames: 0, accessories: 0 };
  ...
  for (const row of lines) {
    if (row.OrderID !== null && row.OrderID !== undefined) orderIds.add(String(row.OrderID));
    ...
    const price = Number(row.InvoicePrice || 0);
    const bucket = getBucket(text(row.GLAccountNumber), text(row.Description));
    buckets[bucket] += price * lineQty;
    if (["lensOther", "lensGlass", "frames", "accessories"].includes(bucket)) {
      qty[bucket] += Math.trunc(lineQty);
    }
    if (bucket === "lensOther" && !CONFIG.glMapping.lens.includes(text(row.GLAccountNumber))) {
      warnings.push(`Unknown GL ${text(row.GLAccountNumber) || "(blank)"} treated as lens value: ${text(row.Description).slice(0, 80)}.`);
    }
  }

  if (qty.lensGlass > 0 && qty.lensOther === 0) buckets.lensGlass += buckets.extras;
  else buckets.lensOther += buckets.extras;

  return { buckets, qty, orderIds, orderRefs, invoiceNos, invoiceDates };
}

function buildItems(aggregate, jobCount, warnings) {
  const items = [];
  const pairsOther = Math.ceil(aggregate.qty.lensOther / 2);
  const pairsGlass = Math.ceil(aggregate.qty.lensGlass / 2);

  if (aggregate.qty.lensOther > 0) {
    items.push({
      hsCode: CONFIG.hsCodes.lensOther.code,
      hsDescription: CONFIG.hsCodes.lensOther.description,
      commercialDescription: "Prescription spectacle lenses (plastic/resin), lab-finished",
      quantity: aggregate.qty.lensOther,
      uom: CONFIG.defaults.unitOfMeasure,
      value: round(aggregate.buckets.lensOther, 2),
      unitCost: round(aggregate.buckets.lensOther / Math.max(1, aggregate.qty.lensOther), 2),
      weightKg: round(pairsOther * CONFIG.weights.lensPairUncut + jobCount * CONFIG.weights.jobPackagingEnvelope, 3)
    });
  }

  if (aggregate.qty.lensGlass > 0) {
    items.push({
      hsCode: CONFIG.hsCodes.lensGlass.code,
      hsDescription: CONFIG.hsCodes.lensGlass.description,
      commercialDescription: "Prescription spectacle lenses (glass), lab-finished",
      quantity: aggregate.qty.lensGlass,
      uom: CONFIG.defaults.unitOfMeasure,
      value: round(aggregate.buckets.lensGlass, 2),
      unitCost: round(aggregate.buckets.lensGlass / Math.max(1, aggregate.qty.lensGlass), 2),
      weightKg: round(pairsGlass * CONFIG.weights.lensPairUncut, 3)
    });
  }

  if (aggregate.qty.accessories > 0) {
    warnings.push("Accessory HS code is not confirmed; review accessory item before queueing.");
    items.push({
      hsCode: "",
      hsDescription: "Eyewear accessories/cases - HS TBC",
      commercialDescription: "Spectacle cases and accessories",
      quantity: aggregate.qty.accessories,
      uom: "Pieces",
      value: round(aggregate.buckets.accessories, 2),
      unitCost: round(aggregate.buckets.accessories / Math.max(1, aggregate.qty.accessories), 2),
      weightKg: round(aggregate.qty.accessories * CONFIG.weights.accessory, 3)
    });
  }

  if (!items.length) warnings.push("No BeSwift item buckets were produced from invoice lines.");
  return items;
}

function getBucket(gl, desc) {
  const lower = String(desc || "").toLowerCase();
  if (CONFIG.glMapping.extrasKeywords.some((keyword) => lower.includes(keyword))) return "extras";
  if (CONFIG.glMapping.lens.includes(gl)) {
    return CONFIG.glMapping.glassKeywords.some((keyword) => lower.includes(keyword)) ? "lensGlass" : "lensOther";
  }
  if (CONFIG.glMapping.extras.includes(gl)) return "extras";
  if (CONFIG.glMapping.frames.includes(gl)) return "frames";
  if (CONFIG.glMapping.accessories.includes(gl)) return "accessories";
  if (CONFIG.glMapping.freight.includes(gl)) return "freight";
  return "lensOther";
}
...
function normalizeJobStatus(value) {
  ...
  return ["claimed", "logging_in", "filling", "filled_review", "error", "cancelled"].includes(status) ? status : "filling";
}
