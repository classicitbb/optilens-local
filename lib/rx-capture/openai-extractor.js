const { assistantFromVault } = require("../credential-vault");
const { OPTICAL_FIELDS, extractionJsonSchema, normalizeOpticalOrder } = require("./normalized-order");

const DEFAULT_BASE_URL = "https://api.openai.com/v1";
const DEFAULT_MODEL = "gpt-4.1-mini";
const REQUEST_TIMEOUT_MS = 90000;

const EXTRACTION_INSTRUCTIONS = [
  "Extract only optical prescription and order information visibly present in the supplied images.",
  "Never guess, calculate, transpose, or infer missing prescription powers.",
  "Preserve plus and minus signs and return cylinder exactly as written. Do not convert cylinder notation.",
  "Distinguish OD from OS carefully. Axis must be an integer from 1 through 180 when present.",
  "Do not invent PD, ADD, lens type, frame information, or patient identity.",
  "Printed labels and column headings are not values. A visible ADD, Prism, or Base heading with a blank cell means the value is absent, not unreadable.",
  "ADD is optional for single-vision prescriptions. If the document says single vision, or the ADD cells are clearly blank, return null and do not add ADD paths to missingFields.",
  "Prism and base are optional. When their cells are clearly blank, return null and do not add prism or base paths to missingFields.",
  "For frame.status use TO_BE_TRACED when a physical frame will be supplied or traced later, MEASURED only when actual frame measurements are visible, and UNCUT only when explicitly stated.",
  "Copy the frame model and frame color exactly when they are written or printed (for example on an order sheet, envelope, or frame label). Otherwise return null.",
  "Never invent frame model, color, A, B, DBL, ED, or segment heights. Blank frame fields are optional and must not be added to missingFields.",
  "If the documents say the lenses are supplied by the customer or patient (for example 'own lenses', 'pt supplied', 'cut only'), include that wording in lensRequest.design.",
  "Extract any visible lens type, design, material, option, and coating text. Blank lens detail fields are optional at capture and must not be added to missingFields.",
  "If a mark or value appears to be present but cannot be read reliably, return null and add that exact field path to uncertainFields instead.",
  "Use null for absent values. Add every illegible or ambiguous field path to uncertainFields.",
  "Add absent required fields to missingFields, but never treat an intentionally blank optional field as missing.",
  "Return only the requested structured result."
].join("\n");

// Voice or typed intake. The employee has already checked this text, so it is
// the only source; every rule above still applies, plus the spoken-word rules.
const TEXT_EXTRACTION_INSTRUCTIONS = [
  "Extract only optical prescription and order information stated in the supplied dictation transcript.",
  "The transcript is split into bracketed sections such as [Right eye (OD)] and [Lens]. The section labels come from a keyword splitter and are hints only: decide which eye a value belongs to from the words themselves, and if the words and the label disagree, add the field to uncertainFields.",
  "Never guess, calculate, transpose, or infer missing prescription powers. Return cylinder exactly as spoken. Do not convert cylinder notation.",
  "Copy the sign exactly as spoken. If a sphere or cylinder power was spoken without plus or minus (and is not plano), return it without a sign and add its path to uncertainFields.",
  "If a value is spoken with no eye named, do not assign it to either eye; add the affected paths to uncertainFields. Never assume right was said before left.",
  "Apply a value to both eyes only when the speaker says so explicitly (both, both eyes, OU, same both).",
  "If the speaker corrects themselves or a field is given two different values, return the last value and add the path to uncertainFields.",
  "Axis must be an integer from 1 through 180 when present. ADD is optional for single vision. Prism, base, frame fields and lens details are optional and must not be added to missingFields when not spoken.",
  "For frame.status use TO_BE_TRACED when a frame will follow or be traced, MEASURED only when frame measurements are spoken, and UNCUT only when explicitly stated.",
  "If the lenses are supplied by the customer or patient (own lenses, pt supplied, cut only), include that wording in lensRequest.design.",
  "Put a customer, practice, or account the speaker names in spokenCustomer, else null. Put delivery or handling requests in instructions.",
  "For every non-null value you return, add an evidence entry with the field path and the exact words copied verbatim from the transcript that state it. Do not paraphrase evidence.",
  "Do not invent PD, ADD, lens type, frame information, or patient identity. Use null for anything not spoken.",
  "Return only the requested structured result."
].join("\n");

function textExtractionJsonSchema() {
  const schema = extractionJsonSchema();
  schema.properties.evidence = {
    type: "array",
    items: {
      type: "object",
      additionalProperties: false,
      properties: { field: { type: "string", enum: OPTICAL_FIELDS }, text: { type: "string" } },
      required: ["field", "text"]
    }
  };
  schema.properties.spokenCustomer = { anyOf: [{ type: "string" }, { type: "null" }] };
  schema.required = Object.keys(schema.properties);
  return schema;
}

function loadRxAiConfig(options = {}) {
  const env = options.env || process.env;
  let vault = options.vault;
  if (vault === undefined) {
    try { vault = assistantFromVault(); } catch { vault = null; }
  }
  vault = vault || {};
  // The Credentials Vault is operator-managed and must win over stale machine
  // environment values left by an earlier installation.
  const apiKey = vault.apiKey || env.OPENAI_API_KEY || "";
  const baseUrl = (env.OPENAI_RX_BASE_URL || vault.baseUrl || env.OPENAI_BASE_URL || DEFAULT_BASE_URL).replace(/\/+$/, "");
  const model = env.OPENAI_RX_MODEL || vault.model || env.ASSISTANT_MODEL || DEFAULT_MODEL;
  return { apiKey, baseUrl, model };
}

async function extractPrescriptionFromImages(images, options = {}) {
  if (!Array.isArray(images) || images.length < 1 || images.length > 2) {
    throw userError("One or two prescription images are required.", 400);
  }
  const config = { ...loadRxAiConfig(), ...(options.config || {}) };
  if (!config.apiKey) throw userError("RX Capture AI credentials are not configured.", 503);

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), options.timeoutMs || REQUEST_TIMEOUT_MS);
  const content = [{ type: "input_text", text: "Extract the prescription/order data from these employee-supplied images." }];
  images.forEach((image, index) => {
    const dataUrl = `data:${image.mimeType};base64,${image.buffer.toString("base64")}`;
    // A PDF (a scanned or e-mailed prescription) goes in as a file; the model reads its pages.
    content.push(image.mimeType === "application/pdf"
      ? { type: "input_file", filename: `prescription-${index + 1}.pdf`, file_data: dataUrl }
      : { type: "input_image", image_url: dataUrl, detail: "high" });
  });

  try {
    const response = await (options.fetch || fetch)(`${config.baseUrl}/responses`, {
      method: "POST",
      headers: { "content-type": "application/json", authorization: `Bearer ${config.apiKey}` },
      body: JSON.stringify({
        model: config.model,
        store: false,
        instructions: EXTRACTION_INSTRUCTIONS,
        input: [{ role: "user", content }],
        text: {
          format: {
            type: "json_schema",
            name: "rx_capture_extraction",
            strict: true,
            schema: extractionJsonSchema()
          }
        }
      }),
      signal: controller.signal
    });

    if (!response.ok) {
      throw userError(`Prescription extraction service returned HTTP ${response.status}.`, 502);
    }
    const body = await response.json();
    const outputText = responseOutputText(body);
    if (!outputText) throw userError("Prescription extraction returned no structured result.", 502);
    return normalizeOpticalOrder(JSON.parse(outputText));
  } catch (error) {
    if (error.name === "AbortError") throw userError("Prescription extraction timed out.", 504);
    if (error instanceof SyntaxError) throw userError("Prescription extraction returned invalid structured data.", 502);
    throw error;
  } finally {
    clearTimeout(timer);
  }
}

// Returns { order, evidence, spokenCustomer }. The caller applies the
// deterministic voice rules; this only asks for a structured proposal.
async function extractPrescriptionFromText(text, options = {}) {
  const transcript = String(text || "").trim();
  if (!transcript) throw userError("The dictation is empty.", 400);
  const config = { ...loadRxAiConfig(), ...(options.config || {}) };
  if (!config.apiKey) throw userError("RX Capture AI credentials are not configured.", 503);

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), options.timeoutMs || REQUEST_TIMEOUT_MS);
  try {
    const response = await (options.fetch || fetch)(`${config.baseUrl}/responses`, {
      method: "POST",
      headers: { "content-type": "application/json", authorization: `Bearer ${config.apiKey}` },
      body: JSON.stringify({
        model: config.model,
        store: false,
        instructions: TEXT_EXTRACTION_INSTRUCTIONS,
        input: [{ role: "user", content: [{ type: "input_text", text: `Extract the prescription/order data from this employee-checked dictation transcript.\n\n${transcript}` }] }],
        text: { format: { type: "json_schema", name: "rx_capture_text_extraction", strict: true, schema: textExtractionJsonSchema() } }
      }),
      signal: controller.signal
    });
    if (!response.ok) throw userError(`Prescription extraction service returned HTTP ${response.status}.`, 502);
    const outputText = responseOutputText(await response.json());
    if (!outputText) throw userError("Prescription extraction returned no structured result.", 502);
    const raw = JSON.parse(outputText);
    return {
      order: normalizeOpticalOrder(raw),
      evidence: Array.isArray(raw.evidence) ? raw.evidence : [],
      spokenCustomer: typeof raw.spokenCustomer === "string" ? raw.spokenCustomer : null
    };
  } catch (error) {
    if (error.name === "AbortError") throw userError("Prescription extraction timed out.", 504);
    if (error instanceof SyntaxError) throw userError("Prescription extraction returned invalid structured data.", 502);
    throw error;
  } finally {
    clearTimeout(timer);
  }
}

function responseOutputText(body) {
  if (typeof body?.output_text === "string") return body.output_text;
  for (const item of body?.output || []) {
    for (const content of item?.content || []) {
      if (content?.type === "output_text" && typeof content.text === "string") return content.text;
    }
  }
  return "";
}

function userError(message, statusCode) {
  return Object.assign(new Error(message), { statusCode });
}

module.exports = { extractPrescriptionFromImages, extractPrescriptionFromText, loadRxAiConfig, responseOutputText, textExtractionJsonSchema };
