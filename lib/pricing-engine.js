...
(function (root, factory) {
  ...
  function saneMargin(value, fallback) {
    const n = Number(value);
    if (!Number.isFinite(n) || n <= 0 || n >= 0.95) return fallback;
    return n;
  }

  function normalizeOptions(opts) {
    const o = Object.assign({}, DEFAULTS, opts);
    o.floorMargin = saneMargin(o.floorMargin, DEFAULTS.floorMargin);
    o.minMargin = saneMargin(o.minMargin, o.floorMargin);
    if (!Number.isFinite(Number(o.rounding)) || Number(o.rounding) < 0) o.rounding = DEFAULTS.rounding;
    if (!Array.isArray(o.priority)) o.priority = DEFAULT_PRIORITY;
    if (!Array.isArray(o.excluded)) o.excluded = [];
    return o;
  }
   ...
   * CONFIRM. When no supplier clears the floor, still return a confirmation
   * state so the caller can save a manual below-floor override deliberately.
   */
  function evaluateOverride(combo, enteredPriceUSD, opts) {
    ...
    return {
      ...
      allowUnsafe: acceptable.length === 0,
      ...
      message: acceptable.length
        ...
        : `At $${enteredPriceUSD} no approved supplier clears ${Math.round(o.minMargin * 100)}% margin. Save anyway as a manual below-floor override?`,
    };
  }
