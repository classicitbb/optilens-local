...
const TABLES = [
  ...
  },
  {
    ...
    strategy: "incremental",
    retentionDaysConfig: "retentionDays",
    retentionColumns: ["RecordCreated", "LastUpdated"]
  },
  {
    name: "Orders",
    strategy: "incremental",
  ...
  },
  {
    name: "RxArchive",
    strategy: "incremental",
    initialRangeColumn: "RxDate",
    initialRangeDaysConfig: "initialOrderDays",
    retentionDaysConfig: "retentionDays",
    retentionColumns: ["ShipDate", "RxDate"]
  },
  {
    name: "Invoices",
    strategy: "incremental",
    retentionDaysConfig: "retentionDays",
    retentionColumns: ["InvoiceTime", "RecordCreated", "LastUpdated"]
  },
  {
    name: "InvoiceLines",
    strategy: "incremental",
  ...
  {
    name: "FinARSalesJournal",
    strategy: "incremental",
    retentionDaysConfig: "retentionDays",
    retentionColumns: ["ShipTime", "InvoiceTime", "ReceivedTime", "LastUpdated"]
  },
  {
    name: "FinARStatements",
    strategy: "incremental",
    retentionDaysConfig: "retentionDays",
    retentionColumns: ["StatementDate", "ToDate", "LastUpdated"]
  },
  {
    name: "FinARStatementItems",
    strategy: "incremental",
    retentionDaysConfig: "retentionDays",
    retentionColumns: ["PostDate", "TransDate", "LastUpdated"]
  }
];
...
function retentionDays(table) {
  if (!table.retentionDaysConfig) return 0;
  const days = Number(getConfig().sourceMirror[table.retentionDaysConfig]);
  return Number.isFinite(days) && days > 0 ? days : 0;
}
...
function buildRetentionPredicate(table, columns, quoteColumn, cutoffExpression) {
  const columnNames = new Set(columns.map((column) => String(column.name).toLowerCase()));
  const parts = [];
  for (const column of table.retentionColumns || []) {
    if (columnNames.has(String(column).toLowerCase())) {
      parts.push(`${quoteColumn(column)} >= ${cutoffExpression}`);
    }
  }
  if (table.retainOpenShipments && columnNames.has("shipped")) {
    parts.push(`(${quoteColumn("Shipped")} = 0 OR ${quoteColumn("Shipped")} IS NULL)`);
  }
  return parts.length ? parts.map((part) => `(${part})`).join(" OR ") : null;
}

function buildMirrorRetentionDeleteSql(table, columns) {
  if (!retentionDays(table)) return null;
  const predicate = buildRetentionPredicate(table, columns, quoteIdent, "@cutoff");
  return predicate
    ? `DELETE FROM dbo.${quoteIdent(table.name)} WHERE NOT (${predicate});`
    : null;
}
...
function buildZenSelectQuery(table, columns, { full = false, watermark = null, now = new Date() } = {}) {
  ...
  const retentionWhere = days
    ? buildRetentionPredicate(table, columns, (name) => `"${name}"`, formatZenTimestamp(retentionCutoff(now, days)))
    : null;

  if (full) {
    return {
      query: `SELECT ${selectList} FROM "${table.name}"${retentionWhere ? ` WHERE ${retentionWhere}` : ""}`,
      sourceComplete: true,
      boundedInitial: false,
      retentionBounded: !!retentionWhere,
      retentionDays: retentionWhere ? days : null
    };
  }
  ...
  if (retentionWhere) {
    return {
      query: `SELECT ${selectList} FROM "${table.name}" WHERE ${retentionWhere}`,
      sourceComplete: true,
      boundedInitial: true,
  ...
  return {
    query: `SELECT ${selectList} FROM "${table.name}"`,
    sourceComplete: true,
    boundedInitial: false,
    retentionBounded: false,
    retentionDays: null
...
async function syncFullTable(pool, connection, table, columns, { now = new Date() } = {}) {
  const select = buildZenSelectQuery(table, columns, { full: true, now });
  ...
  try {
    ...
    fetchErrors = await streamZenQuery(connection, select.query, async (batch) => {
  ...
  return {
    ...
    retentionBounded: select.retentionBounded || false,
    retentionDays: select.retentionDays || null,
    fetchErrors
  };
}
...
async function syncIncrementalTable(pool, connection, table, columns, pkColumns, { full = false } = {}) {
  ...
  if (!pkColumns.length) {
    ...
    return syncFullTable(pool, connection, table, columns, { now });
  }
  ...
  const select = buildZenSelectQuery(table, columns, { full, watermark, now });
  ...
  try {
    ...
    if (pruneSql) {
      const prune = await new sql.Request(transaction)
        .input("cutoff", sql.DateTime2(3), retentionCutoff(now, retentionDays(table)))
        .query(pruneSql);
      prunedRows = (prune.rowsAffected || []).reduce((sum, value) => sum + Number(value || 0), 0);
    }
    await transaction.commit();
  } catch (error) {
    try { await transaction.rollback(); } catch {}
  ...
  try {
    ...
    for (const table of TABLES) {
      ...
      try {
        ...
        if (result.retentionBounded) {
          entry.retentionBounded = true;
          entry.retentionDays = result.retentionDays;
        }
        if (result.prunedRows) {
          entry.prunedRows = result.prunedRows;
        }
