...
function createRxCaptureService(dependencies = {}) {
  ...
  async function saveResolution(orderId, payload, actor) {
    ...
    const resolved = normalizeResolution({ ...payload?.resolution, customerNumber: existing.customer_account, remoteOperator: actor.username || actor.displayName || "" });
  ...
  return { approveOrder, createOrder, getApprovalOrder, getOrder, listApprovalQueue, listOrders, processOrder, releaseOrder, reprocessOrder, saveResolution, stageOrder, suggestAlias, updateOrder };
}
