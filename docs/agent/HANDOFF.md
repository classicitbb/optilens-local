...
Commercial Invoice previews now reserve a transparent signature image area above a distinct signing line, with signer text below. This prevents an authorisation PNG from straddling the line and renders any white backdrop invisibly on the white document while retaining the signature strokes. The local renderer change is not deployed.
