...
async function buildDashboard(deviceId, userId = null) {
  ...
  const hydratedTiles = catalog
    ...
    .map((tileRow) => hydrateTile(tileRow, stats, appDbHealth, sourceDbHealth, config, moduleHrefByCode));
    ...
    .query(`
      ...
      SELECT
        ...
        CASE module_code
          ...
          WHEN N'rx-capture' THEN N'Photograph prescriptions and review normalized optical order data.'
...
function hydrateTile(row, stats, appDbHealth, sourceDbHealth, config, moduleHrefByCode = new Map()) {
...
function metricForTile(key, stats, appDbHealth, sourceDbHealth, config) {
  ...
  const values = {
    "open-source-shipments": { value: "MSSQL", detail: "Innovations shipments where Shipped = 0", state: sourceDbHealth.state },
