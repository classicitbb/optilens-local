...
const { INVENTORY_DRILLS } = require("./inventory");
const { getCostListGaps } = require("./cost-lists");
...
async function getDrill(kind, params = {}) {
  // Inventory drills resolve first and manage their own connection: they read
  // live Innovations MSSQL (the mirror has no stock tables) and serve from a
  // cached universe rather than re-scanning 1.5M LensItem rows per drill.
  const inventoryHandler = INVENTORY_DRILLS[kind];
  if (inventoryHandler) {
    const data = await inventoryHandler(params);
...
const HANDLERS = {
  ...
  async "zero-cost-lines"(pool) {
    const result = await pool.request()
      ...
      .query(`
        SELECT TOP (${ROW_LIMIT})
          ...
          -- CategoryType 1 is Lens. 16 is Package, not a lens.
          CASE WHEN d.CategoryType = 1 THEN N'lens' ELSE N'add-on' END AS lineKind,
        ...
        WHERE a.ShippedTime >= DATEADD(day, -@days, CAST(SYSDATETIME() AS date))
          ...
          -- Packages (16) excluded: zero-cost by design, not a defect.
    ...
    return {
      ...
      rows,
      next: { kind: "invoice-audit", carry: ["invoiceId"] }
    };
  },

  /** Full read-only invoice context for a line flagged by the zero-cost audit. */
  async "invoice-audit"(pool, params) {
    const invoiceId = intParam(params.invoiceId, "invoiceId");
    const result = await pool.request()
      .input("invoiceId", sql.Int, invoiceId)
      .query(`
        SELECT
          c.InvoiceID AS invoiceId, c.OrderID AS orderId, c.InvoiceTime AS invoiceTime,
          c.SubTotal AS subtotal, c.TaxAmount AS taxAmount, c.Total AS total,
          a.CustomerOrdReference AS rxNumber, a.ShippedTime AS shippedAt,
          cu.AccountNumber AS accountNumber, cu.CustomerName AS customerName,
          ot.OrderTypeName AS orderTypeName
        FROM dbo.Invoices c
        LEFT JOIN dbo.Orders a ON a.OrderID = c.OrderID
        LEFT JOIN dbo.Customers cu ON cu.CustomerID = COALESCE(a.BillToID, c.CustomerID, a.CustomerID)
        LEFT JOIN dbo.OrderTypes ot ON ot.OrderType = a.OrderType
        WHERE c.InvoiceID = @invoiceId;

        SELECT
          d.SKU AS sku, d.Description AS description, d.CategoryType AS categoryType,
          CAST(COALESCE(d.Quantity, 0) AS decimal(18, 2)) AS quantity,
          CAST(COALESCE(d.InvoicePrice, 0) AS decimal(18, 2)) AS unitPrice,
          CAST(COALESCE(d.CostPrice, 0) AS decimal(18, 2)) AS unitCost,
          CAST(COALESCE(d.InvoicePrice, 0) * COALESCE(d.Quantity, 0) AS decimal(18, 2)) AS lineRevenue,
          CAST(COALESCE(d.CostPrice, 0) * COALESCE(d.Quantity, 0) AS decimal(18, 2)) AS lineCost,
          CASE WHEN COALESCE(d.InvoicePrice, 0) * COALESCE(d.Quantity, 0) = 0 THEN NULL
               ELSE CAST((COALESCE(d.InvoicePrice, 0) - COALESCE(d.CostPrice, 0)) /
                 NULLIF(COALESCE(d.InvoicePrice, 0), 0) * 100 AS decimal(18, 2)) END AS marginPct
        FROM dbo.InvoiceLines d
        WHERE d.InvoiceID = @invoiceId
          AND COALESCE(d.Suppressed, 0) = 0
        ORDER BY d.RecordID;
      `);

    const invoice = result.recordsets[0][0];
    if (!invoice) {
      throw Object.assign(new Error("Invoice audit record was not found."), { statusCode: 404 });
    }
    const rows = result.recordsets[1].map((r) => ({
      sku: trim(r.sku),
      description: trim(r.description) || trim(r.sku),
      categoryType: r.categoryType == null ? null : String(r.categoryType),
      quantity: n(r.quantity),
      unitPrice: n(r.unitPrice),
    ...
    return {
      title: `Invoice audit · ${invoiceId}`,
      subtitle: `${trim(invoice.accountNumber) || "—"} · ${trim(invoice.customerName) || "Unknown customer"} · ${trim(invoice.rxNumber) || "No RX"}`,
      summary: [
        { label: "Invoice date", value: invoice.invoiceTime, format: "date" },
        { label: "Shipped", value: invoice.shippedAt, format: "date" },
        { label: "Subtotal", value: n(invoice.subtotal), format: "money" },
        { label: "Total", value: n(invoice.total), format: "money" }
      ],
      columns: [
        { key: "sku", label: "SKU", format: "text" },
        { key: "description", label: "Description", format: "text" },
        { key: "categoryType", label: "Category", format: "text" },
        { key: "quantity", label: "Qty", format: "int", align: "right" },
        { key: "unitPrice", label: "Unit price", format: "money", align: "right" },
      ...
      rows,
      emptyText: "This invoice has no visible invoice lines."
    };
  },
  ...
  /**
   * Template cells a supplier cost list is missing. Owns its own pool handling in
   * cost-lists.js because it must probe for the pricelist tables first — they are not
   * carried by the mirror.
   */
  async "cost-list-gaps"(pool, params) {
    return getCostListGaps({ priceListId: params.priceListId, rowLimit: ROW_LIMIT });
  },
