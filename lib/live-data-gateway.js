const { getAppPool, getMirrorPool, getSourcePool } = require('./db');
...
function normalizeRequest(request) {
  ...
  const target = {
    ...
    sourceBackend: request.source_backend === 'mirror' ? 'mirror' : 'active',
  };
...
async function getRequestSourcePool(target) {
  return target.sourceBackend === 'mirror' ? getMirrorPool() : getSourcePool();
}
...
async function getInnovationsCustomerAccount(target) {
  const pool = await getRequestSourcePool(target);
