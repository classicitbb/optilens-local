...
function buildOrder(payload, config) {
  ...
  // The web form has no frame-tracer hardware, so we never have real
  // trace_start/trace_end geometry to send (see docs/rx-format-field-map.md).
  // Enum values below are verified against real LabLink .rx samples:
  //   uncut job              -> source "NO TRACE - UNCUT", status "UNCUT",   tracing "NO TRACE"
  //   enclosed + real tracer -> source "TRACE - UNCUT",    status "ENCLOSED",tracing "TRACED" (+ trace block)
  // "FRAME TRACE" (the old value here) never appears in any real sample and
  // would misrepresent an edged job as having trace geometry it doesn't have.
  // The enclosed-but-not-traced case (frame shipped, no digitizer data) has
  // no confirmed real sample — best-guess mapping below; flag to Russell if
  // Innovations rejects/mishandles it.
  const orderFrame = {
