...
//   C. STOCK SOLD   stock lenses invoiced through fulfilment (InvoiceLines -> SKU -> Add_Cyl)
//
...
async function getAddPowerTrends(windowMonths) {
  ...
  const result = await request.query(`
    ...
    -- C. STOCK SOLD: stock lenses are invoiced through Fulfillment (OrderType
    -- 6), not the legacy Stock / Stock Debit order types.  Match the invoiced
    -- SKU to its OPC so the add comes from the actual stock lens.
    ...
    WHERE i.InvoiceTime >= @windowStart
      AND il.CategoryType = 1 AND o.OrderType = 6 AND li.Add_Cyl > 0
