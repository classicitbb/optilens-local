...
async function composeShipmentItems(lines, warnings) {
  ...
  // Group same commodity/price/HS/origin lines together regardless of which
  // order or invoice reference they came from. This is the single partition
  // used for both the printed commercial invoice and the BeSwift certificate
  // (see buildCommercialInvoiceLines / buildPayloadFromShipment below), so the
  // two always foot to the same totals \u2014 previously the certificate grouped
  // across refs while the invoice kept every ref apart, which both produced
  // duplicate-looking invoice lines that should have been one, and (combined
  // with the certificate being computed independently of invoice overrides)
  // let the two documents disagree on value (root-caused live 2026-08-13,
  // shipment 11357). `refs` carries every merged order's reference so the
  // grouping stays traceable instead of silently dropping references.
  ...
  const items = [...grouped.values()].map((item) => ({ ...item, ref: item.refs.join(", ") }));
  return { items, freightTotal, jobCount: byOrder.size, orderRefs, poRefs, invoiceNos, invoiceDates };
}

// Applies operator-saved commercial-invoice line overrides (price/quantity/
// amount corrections) on top of the freshly composed items, once, so both the
// printed commercial invoice and the BeSwift certificate see the same
// corrected values (see composeShipmentItems's grouping comment above).
async function buildCommercialInvoiceLines(session, lines, warnings) {
  ...
  const items = composed.items.map((item) => {
    const override = overrides.get(item.lineKey) || {};
    ...
    return { ...item, override, unitPrice, quantity, amount };
  });
