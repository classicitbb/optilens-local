...
function normMaterial(name, matcol) {
  const s = `${name || ''} ${matcol || ''}`.toLowerCase();
  if (/\b1\.74\b/.test(s)) return '1.74';
  if (/\b1\.67\b/.test(s)) return '1.67';
  if (/\b1\.60\b|1\.6 /.test(s)) return '1.60';
  if (/\b1\.59/.test(s)) return '1.59';
  if (/trivex|1\.53/.test(s)) return 'TRIVEX';
  if (/\bpoly\b|polycarb/.test(s)) return 'POLY';
  if (/1\.565|1\.56\b/.test(s)) return '1.56';
  if (/\bglass\b/.test(s)) return 'GLASS';
  if (/\b1\.50\b|plastic|photochromic 1\.50|cr-?39/.test(s)) return '1.50';
  return null;
}
function normTreatment(name) {
  const n = (name || '').toLowerCase(); const brown = /brown|amber/.test(n);
  ...
  if (/photo/.test(n) || /\bpch\b/.test(n)) return brown ? 'Photochromic - Brown' : 'Photochromic - Gray';
  if (/uv\s*420|blue|bluv/.test(n)) return 'UV420';
  return 'Clear';
}
const TIER_MAP = {
  'Progressive|Endless Steady': 'Progressive - Best', 'Progressive|Essential Steady': 'Progressive - Better',
  'Progressive|Classic PAL': 'Progressive - Good', 'Progressive|Endless Offc 1.3m': 'Specific Use - Office',
  'Progressive|Reader II': 'Specific Use - Office', 'Progressive|Endless Plus': 'Anti-Fatigue',
  'Progressive|AcomodaII': 'Anti-Fatigue', 'Progressive|Endless Sport': 'Specific Use - Sport',
  'Single Vision|Endless SV': 'Single Vision - HD', 'Single Vision|Camber': 'Single Vision - HD',
  'Single Vision|AntiFatigue': 'Anti-Fatigue', 'Single Vision|AcomodaII': 'Anti-Fatigue',
  'Single Vision|Regular': 'Single Vision - Regular', 'Single Vision|Classic SV': 'Single Vision - Regular',
  'Single Vision|Pro Regular': 'Single Vision - Regular', 'Single Vision|NuPolar': 'Single Vision - Regular',
  'Single Vision|Polarized': 'Single Vision - Regular',
  'Bifocal|Endless BF Round': 'Specific Use - Bifocal', 'Bifocal|Flat Top 28': 'Specific Use - Bifocal',
  'Bifocal|Flat Top 35': 'Specific Use - Bifocal', 'Bifocal|Round 28': 'Specific Use - Bifocal',
  'Bifocal|Round 24': 'Specific Use - Bifocal', 'Bifocal|Round 22': 'Specific Use - Bifocal',
  'Bifocal|C25': 'Specific Use - Bifocal', 'Bifocal|C28': 'Specific Use - Bifocal',
  'Bifocal|D45': 'Specific Use - Bifocal', 'Bifocal|Executive': 'Specific Use - Bifocal',
  'Trifocal|Flat Top 7x28': 'Specific Use - Bifocal', 'Trifocal|Flat Top 8x35': 'Specific Use - Bifocal',
};
const APPROVED = ['TOG Rx Lab', 'Optex Laboratories', 'Vision Rx Lab', 'SkyLab'];
const r2 = (n) => Math.round(n * 100) / 100;

function combosFromRows(rows) {
  const coverage = {}, meta = {}, prov = {}, supRows = {};
  ...
  for (const r of rows) {
    ...
    const tier = TIER_MAP[lk]; if (!tier) continue;
    const material = normMaterial(r.name, r.material); if (!material) continue;
  ...
  const combos = Object.entries(coverage).map(([key, sup]) => {
    ...
    return { key, ...meta[key], suppliers: sup, provenance: prov[key], anchorCost: anchor[1], anchorSupplier: anchor[0], cheapestCost: cheap[1], cheapestSupplier: cheap[0], supplierCount: items.length };
  });
...
async function pull(creds, { write = true } = {}) {
  const raw = await fetchLenses(creds);
  const rows = raw.map(mapRow);
  const { combos, coverage, approved, mapped, supRows } = combosFromRows(rows);
  const sources = {
