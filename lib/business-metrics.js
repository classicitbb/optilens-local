...
async function getBusinessMetrics() {
  ...
  const sources = { appDb: appHealth, sourceMssql: sourceHealth };
...
async function getSourceKpis() {
  ...
  const result = await pool.request().query(`
    ...
    SELECT
      ...
      -- 1=Lens. CategoryType 16 is Package — zero-cost by design, so it must not be
      -- classed as a lens or it inflates the zero-cost KPI. See dbo.CategoryTypes.
      CASE
