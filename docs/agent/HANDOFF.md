...
Affected files: `lib/rx-capture/{order-builder,service,routes}.js`, `public/rx-capture.{html,js}`, `public/styles/pages/rx-capture.css`, `test/rx-capture*.test.js`, and this handoff/project knowledge record. Focused RX tests pass 20/20; changed-file syntax checks and `git diff --check` pass. The guarded host updater deployed revision `c546d77`, ran its full suite successfully (221/221), and monitor-harness verification reported all systems online. No migration, live extraction, staged file, release, or external source-system write was performed.

Next action: in authenticated external Edge/Chrome use an approved non-production image to verify direct typing, missing-value warnings, optional coating choice, same-user submit confirmation, and the final Innovations-release status. That live check performs a billable extraction and a file-drop release, so it requires explicit patient-data and release approval.
...
RX Capture Milestone 2 is deployed in revision `536a0ad`. An employee can save an exact production configuration for a `READY_FOR_REVIEW` capture: customer number/sequence, ship name, source-validated 13-digit lens alias, optional exact coating/add-on SKUs, explicit uncut or edged frame mode, mounting code, and instructions. The deterministic builder validates the reviewed optical values, configured lens profile, frame dimensions, segment heights where required, exact catalogue items, and a patient name before it reserves identifiers. A different employee with `rx-capture.approve` must then approve, which creates one immutable generated-order record containing the rendered `.rx` content, SHA-256, and snapshot. An authorized stager can write only that content to the local staging directory. The RX Capture routes contain no release call and never touch the Innovations incoming folder.

Milestone 2 affected files are `database/044-rx-capture-milestone-2.sql`, `lib/rx-capture/order-builder.js`, `lib/rx-capture/service.js`, `lib/rx-capture/routes.js`, `lib/auth.js`, `lib/migrations.js`, `public/rx-capture.html`, `public/rx-capture.js`, `public/styles/pages/rx-capture.css`, and RX Capture tests. Focused builder and RX Capture tests pass (16/16), as do changed-file syntax checks and `git diff --check`. The guarded host update applied migration `044-rx-capture-milestone-2.sql`, passed its smoke check and full suite (206/206), restarted successfully, and ended with monitor verification reporting all systems online. No model call, staged file, release, source-system write, or real image was used for Milestone 2.

RX Capture Milestones 3 and 4 are deployed in revision `9cad5ec`. An authorized stager writes only hash-verified immutable approved content to staging through same-directory temporary-file replacement. A separate `rx-capture.release` permission then releases only a verified staged file, retains the archive copy, and records `RX_RELEASED_TO_INNOVATIONS`; no release was exercised during deployment. Active catalogue choices now exclude inactive lenses, separate coating from lens-option matching, and present staff with readable type/material/design/color choices that set the exact alias. Migrations `046-rx-capture-milestone-4-release.sql` and `047-file-drop-destinations.sql` applied successfully. Integrations → Innovations Sync now supports active customer- and purpose-scoped file routes plus a non-writing read/write path test; the active default RX route was configured and verified reachable. The guarded updater smoke check and full host suite passed 217/217, and the final monitor harness reported all systems online.

RX Capture review refinements are deployed in revision `c73aea6`: the recent-orders listener no longer receives `Array.map` metadata as an event handler; capture now requires a source-validated ERP customer selected before the image; patient names are normalized to `LAST, FIRST`; the review form uses one flat OD/OS prescription table with PD and optical-center-height values; frame fields follow patient fields; and production configuration is gated on an explicit review save. The server persists the chosen customer and review confirmation through additive migration `045-rx-capture-review-workflow.sql`, forces the remote operator to the authenticated user, and provides a guarded catalogue suggestion (photochromic without a colour defaults to gray; clear defaults to SR-coated) that still requires confirmation of the exact alias. Affected files are `database/045-rx-capture-review-workflow.sql`, `lib/migrations.js`, `lib/rx-capture/{alias-resolver,normalized-order,routes,service}.js`, `server.js`, `public/rx-capture.{html,js}`, `public/styles/pages/rx-capture.css`, and `test/rx-capture.test.js`. The guarded host update applied migration 045, passed smoke and its full 210/210 suite, restarted the application, and ended with `monitor-harness verify` reporting all systems online. No patient-image capture, source-system write, staging, or release occurred. Authenticated external-browser acceptance remains pending; use direct typing in Edge or Chrome for the customer search and patient-name checks.
